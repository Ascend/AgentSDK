# 训推单步异步分离模式使用指南（One Step Off 策略）
...
| 参数 | 说明                                      |
|------|-----------------------------------------|
| work_mode | 设为 `one_step_off` 表示单步异步分离模式            |
...
#### 3.2 单步异步分离模式（One Step Off策略）关键配置项
...
例如：`tensor_parallel_size=8, data_parallel_size=2`，则推理需要 16 卡，在Atlas 900 A3 SuperPoD超节点（16 × 64 GB）上需要 1 个节点。
