...
混合批次调度（Mixed Batch Scheduling）是Agent SDK在训推分离模式下提供的一种调度优化策略。该策略在单步异步分离模式的基础上，让推理端在一次权重更新后连续处理多个训练batch，将推理结果流式发送给训练端，从而减少权重切换频率，但相较于单步异步分离模式，由于训推相差多个迭代，故精度会略有下降。
...
- 训练策略：基于One Step Off策略，训练使用上一轮或更早的策略产出的轨迹数据。混合批次调度通过批次合并（一次权重更新后推理端连续产出多个batch）与版本控制机制，确保权重版本与训练数据版本在off-policy下的正确对应。
...
- 训练端初始化时，`initialize_rollout()`调用`send_initial_batch_groups_to_rollout()`，向推理队列填充`init_num_group_batches`（2个）prompt数据，建立初始流水线。
- 每个生成周期开始时，推理端先按版本控制同步最新权重到推理引擎（首个周期沿用初始权重），随后在一个生成周期内**连续生成`hybrid_batch_num`（2）个batch的数据**：仍按单个batch（`global_batch_size`个prompt）粒度逐批生成，但批次之间**不等待权重同步**，直到本周期所有batch生成完毕；下一次权重同步发生在下一个生成周期开始时。
- 每收集够一个子batch的轨迹数据，推理端即刻将推理结果发送给训练端，训练端通过`receive_minibatch()`→`put_minibatch_to_queue()`放入训练队列。
- 训练端收到训练batch后开始训练，每轮训练完都将权重导出为iter_0X。
- 训练端内的权重转换组件通过`enable_version_control`校验版本（`required_version = max_possible_version - 1`），仅版本匹配时执行权重格式转换并落盘（weights_0X），中间版本自动跳过；推理端不感知该转换动作，只在生成周期开始时轮询加载最新版本的weights_0X。

### 适用场景

- 分离式部署，权重切换开销较大（如网络传输、文件I/O较慢）。
- 减少推理端权重切换频率，提升NPU利用率。
- 对off-policy偏差容忍度较高，允许推理端使用同一份权重处理多个batch。

> [!TIP] 提示
> 如果集群资源有限，希望训练和推理共享同一组卡，可参考[训推共卡模式使用指南](02_hybrid.md)。
> 如果希望运行标准的分离模式（每次权重更新处理一个batch），详情可参考[训推单步异步分离使用指南](03_one_step_off.md)。
> 如果需要提升训练速度，减少长尾，建议参考[训推全异步分离使用指南](04_fully_async.md)

### 与其它模式的区别

| 特性                | 共卡模式（Hybrid） | 单步异步分离模式（One Step Off） | 混合批次调度（Mixed Batch Scheduling） |
|-------------------|--------------|------------------------|--------------------------------|
...
> [!IMPORTANT] 关键配置
> 混合批次调度的hosts.conf配置与分离模式完全相同，均为不配置`infer_master_index`。系统根据`VC_TASK_INDEX < MASTER_TRAIN_INDEX`判定节点为推理节点，`VC_TASK_INDEX >= MASTER_TRAIN_INDEX`判定为训练节点。详情可参考[训推单步异步分离使用指南](03_one_step_off.md#步骤-1修改-hostsconf)。
...
train_config_name=verl_train_mixed_A3_t16_qwen36_27b_math_fsdp

# 推理 YAML 文件（分离模式必须配置）
infer_config_name=vllm_infer_i16_qwen36_27b
```
...
以verl后端、Qwen3.6-27B模型为例，训练配置文件路径：`configs/train/verl_train_mixed_A3_t16_qwen36_27b_math_fsdp.yaml`
...
| `verl_conf.actor_rollout_ref.model.path`            | 模型权重路径                                                  | `/data/weights/Qwen3.6-27B`                      |
| `train_instances.rollout_config.llm_tokenizer_path` | 分词器路径（与模型路径一致）                                          | `/data/weights/Qwen3.6-27B`                      |

#### 3.2 混合批次调度关键配置项

以下配置项是混合批次调度的核心参数，用于控制批次合并与权重版本校验（以Qwen3.6-27B模型为例，实际请根据模型规格调整）。

**train_instances中work_mode必须为one_step_off：**

```yaml
train_instances:
  - name: RL-QWEN36-27B-WITH-MATH
    executor_kwargs:
...
| 参数                       | 说明                                                                        | 推荐值                             |
|--------------------------|---------------------------------------------------------------------------|---------------------------------|
| `init_num_group_batches` | 推理端首次加载的prompt数量                                                          | 推荐为2                            |
| `hybrid_batch_num`       | 一次权重更新后推理端连续处理的batch数量。值越大，权重切换频率越低，但off-policy偏差越大                       | 推荐为2，且与init_num_group_batches相等 |
...
分离模式需要单独配置推理 YAML 文件，路径：`configs/infer/vllm_infer_i16_qwen36_27b.yaml`

#### 4.1 必须修改的路径参数

| 参数                 | 说明     | 示例                          |
|--------------------|--------|-----------------------------|
| `infer_model_path` | 模型权重路径 | `/data/weights/Qwen3.6-27B` |

#### 4.2 推理关键配置项

```yaml
# 模型名称
infer_model_name: Qwen3.6-27B
...
例如：`tensor_parallel_size=4, data_parallel_size=4`，则推理需要16卡，在A3机器（16卡/节点）上需要1个节点。
...
- 训练端`initialize_rollout()`调用`send_initial_batch_groups_to_rollout()`，向推理队列填充`init_num_group_batches`（2 个）prompt数据。
- 推理端启动，训练端部署完毕并完成初始化，一次性生成`init_num_group_batches`（2 个）批次数据。
...
- 训练端收到训练batch后开始训练，每轮训练完都将权重导出为iter_0X。
- 训练端内的权重转换组件通过`enable_version_control`校验版本：`required_version = max_possible_version - 1`。
- 只有版本匹配时才执行权重格式转换并落盘（weights_0X），中间版本自动跳过；推理端不感知该转换动作，只在生成周期开始时轮询加载最新版本的weights_0X。
...
| 概念                     | 说明                                                                         |
|------------------------|----------------------------------------------------------------------------|
| `max_possible_version` | 初始为0，推理端每个生成周期开始时+=`hybrid_batch_num`，表示当前批次组覆盖的最大版本号                      |
| `required_version`     | `max_possible_version - 1`，即推理端期望接收的权重版本                                   |
| `input_weight_version` | 训练端每轮训练后导出的权重版本，即权重转换流程本次接收到的输入版本                                          |
| 版本匹配                   | `input_weight_version` == `required_version`时，执行训练权重落盘（weights_0X）以及推理权重更新 |
| 版本跳过                   | `input_weight_version` != `required_version`时，跳过本次转换，推理端继续使用旧权重            |

以 `hybrid_batch_num=2` 为例，训练端每隔两轮存盘权重，推理端检查并接受特定版本进行同步：

```text
iter_1 → input_weight_version=1 → required_version=1 匹配 → 转换生成 weights_1 存盘
iter_2 → input_weight_version=2 → required_version=3 不匹配 → 跳过（不转换）
iter_3 → input_weight_version=3 → required_version=3 匹配 → 转换生成 weights_3 存盘
iter_4 → input_weight_version=4 → required_version=5 不匹配 → 跳过（不转换）
...
```

> 说明：一个生成周期覆盖`hybrid_batch_num`个训练迭代：批次1-2由权重版本0生成（供iter_1~iter_2训练），批次3-4由权重版本1生成（供iter_3~iter_4训练），批次5-6由权重版本3生成（供iter_5~iter_6训练）……推理端仅在每个生成周期开始时加载新权重，同一周期内的多个batch共用同一权重版本，数据版本相对权重转换版本滞后一个周期。
...
|------------------|----------|------------------------------------|
| 训练端导出频率          | 每轮       | 每轮                                 |
| 训练端保存权重文件        | 每轮       | 每2轮（hybrid_batch_num = 2）          |
| 推理端权重转换频率        | 每轮       | 每2轮（hybrid_batch_num = 2）          |
| 推理端跳过中间版本        | 不跳过      | 通过参数`enable_version_control`开启版本控制 |
| 单个推理权重版本处理的数据量   | 1x       | 2x                                 |
| 权重切换次数（200 iter） | 200 次    | 100 次（N=2）                         |

## 源码实现原理

混合批次调度是分离模式的子策略，启动流程与训推单步异步分离模式完全一致（详见[训推单步异步分离使用指南](03_one_step_off.md)），`work_mode=one_step_off`时路由到`verl_async_train`。核心差异在于部署后的批次调度逻辑。
...
| 版本追踪   | [rollout_worker.py](../../../../aura/aura/trainer/rollout/rollout_worker.py)                                | `update_max_version()`在每个生成周期开始时更新`max_possible_version`                                          |
...
每个生成周期内推理端生成的batch数量由`hybrid_batch_num`决定，对应调用链如下：

1. `hybrid_batch_num`（YAML配置）注入`OneStepOffRolloutExecutor`，作为每个生成周期处理的batch数上限。
2. [rollout_executor.py](../../../../aura/aura/trainer/rollout/rollout_executor.py)：`actual_batch_num = min(self.hybrid_batch_num, queue_size)`，确定本周期从推理队列取出的batch数（一般等于`hybrid_batch_num`，受队列存量约束）。
3. [rollout_executor.py](../../../../aura/aura/trainer/rollout/rollout_executor.py)：取出`actual_batch_num`个batch合并后，调用`rollout_worker.generate_sequences.remote(actual_batch_num)`。
4. [rollout_worker.py](../../../../aura/aura/trainer/rollout/rollout_worker.py)：`generate_sequences()`先调用`_do_update_model_weights()`完成一次权重同步（生成周期开始），再进入`multi_batches_generate_sequences()`。
5. [rollout_worker.py](../../../../aura/aura/trainer/rollout/rollout_worker.py)：`multi_batches_generate_sequences()`以`concurrency = len(agent_tasks) / actual_batch_num`为批次粒度流式生成，每凑够一个batch即通过`handle_full_batch_trajectories()`发给训练端，`done_batch_count`累计至`actual_batch_num`后结束本周期——**周期内各batch之间不等待权重同步**。

因此：权重同步频率为每`hybrid_batch_num`个batch一次；同一个权重版本连续生成`hybrid_batch_num`个batch的数据。
...
如果权重切换耗时（训练导出 + 格式转换 + NPU加载）占总每轮时间的比例很小（如<5%），则混合批次调度可能无明显收益。建议先使用单步异步分离模式作为基准，确认权重切换是瓶颈后再启用；如为进一步提高训练速度请尝试使用全异步分离模式。

### Q6：混合批次调度与单步异步分离模式的配置文件有哪些差异

核心差异在于训练 YAML 中的三个参数：

| 参数                       | 单步异步分离模式（One Step Off） | 混合批次调度（Mixed Batch Scheduling） |
|--------------------------|------------------------|--------------------------------|
| `init_num_group_batches` | 1                      | 2                              |
| `hybrid_batch_num`       | 1                      | 2                              |
| `enable_version_control` | false                  | true                           |

其余配置（hosts.conf、base.conf和推理YAML）完全相同。从单步异步分离模式切换到混合批次调度，只需修改上述三个参数即可。
