...
class ProxyAgent(BaseAgent):
    ...
    def _format_observation_as_messages(self, obs: Any) -> list[dict]:
        ...
        if isinstance(obs, dict):
            ...
            elif "task" in obs and "question" in obs["task"]:
    ...
    def update_from_model(self, response_msg: str|dict, **kwargs) -> Action:
        """
        ...
        if isinstance(response_msg, str):
            ...
            tool_calls_dict = response_msg["tool_calls"] if "tool_calls" in response_msg else []
            reasoning = response_msg.get("reasoning_content") or response_msg.get("reasoning")

        assistant_content = response

        # Append assistant message to chat history
        assistant_message = {"role": "assistant", "content": assistant_content, "tool_calls": tool_calls_dict}
        if reasoning:
    ...
    def update_system_prompt(self, system_prompt):
        self.system_prompt = system_prompt
        for role_message in self.messages:
            if role_message["role"] == "system":
                role_message["content"] = system_prompt
                break
