...
class SearchR1Environment(BaseEnv):
    """
    ...
    def __init__(
        ...
        """
        Initialize the SearchR1Environment.
    ...
    def step(self, action: list[dict] | str | dict):
        ...
        if not isinstance(tool_calls, list):
            raise TypeError(f"Expected tool_calls to be a list, got {type(tool_calls).__name__}")
    ...
    @staticmethod
    def from_dict(env_args: dict) -> "SearchR1Environment":
        ...
        return SearchR1Environment(
            task=env_args, tools=tools, tool_map=tool_map, max_steps=max_steps, reward_fn=reward_fn
        )
