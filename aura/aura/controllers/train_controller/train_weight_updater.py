...
class WeightUpdateActor:
    def __init__(self, dispatch_actor, actor_worker):
        self.logger = Loggers(__name__).get_logger()

        self.dispatch_actor = dispatch_actor
        self.actor_worker = actor_worker
        if hasattr(self.actor_worker, 'actor_handlers'):
            self.actor_handlers = self.actor_worker.actor_handlers
        else:
            self.actor_handlers = self.actor_worker._workers
    ...
    def _update_weights_async(self, weight_save_dir: str, iteration: int):
        ...
        self._exports[weight_save_dir] = _ExportTracker(
            iteration=iteration, start_ts=time.time(), expected=len(self.actor_handlers)
        )

        self.logger.info(f"actor handlers: {len(self.actor_handlers)}")

        if hasattr(self.actor_worker, 'actor_handlers'):
            refs = [h.prepare_infer_params_to_cpu.remote(weight_save_dir) for h in self.actor_worker.actor_handlers]
            for i, r in enumerate(refs):
                ray.get(r)
        else:
            self.actor_worker.prepare_infer_params_to_cpu(weight_save_dir)
        self.logger.info(
            f">>> finished updating weights, expected weight count: {self._exports[weight_save_dir].expected}"
        )
