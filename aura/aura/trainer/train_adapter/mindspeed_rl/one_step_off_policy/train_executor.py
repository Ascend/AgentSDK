...
class OneStepOffTrainExecutor(CommonGRPOTrainer):
    ...
    def update_rollout_metrics(self, rollout_metric: Dict[str, Any]) -> None:
        ...
        ray.get(
            self.transfer_dock.update_metrics.remote("traj/res_reward_mean", value=[res_reward_mean], cumulate=True)
        )
        ray.get(self.transfer_dock.update_metrics.remote("traj/res_reward_min", value=[res_reward_min], cumulate=True))
        ray.get(self.transfer_dock.update_metrics.remote("traj/res_reward_max", value=[res_reward_max], cumulate=True))
        ray.get(
            self.transfer_dock.update_metrics.remote(
    ...
    def update_weights_to_rollout_unit(self, last_iteration: bool, iteration: int) -> None:
        ...
        if not last_iteration and (iteration % self.update_weights_interval == 0):
            logger.info("|perf-stat|train| update weights start ...")
            start_time = time.time()
            self.controller.update_rollout_weights(iteration + 1)
            logger.info(f"|perf-stat|train| update weights done, cost: {time.time() - start_time}")
    ...
    def fit(self) -> None:
        ...
        # import os
        # resume_iteration = int(os.getenv("RESUME_ITERATION", '-1'))
        if iteration > 0:
            # 如果断点续训不从0开始，需要清空之前保存的权重文件，并更新rollout unit中的权重文件
            logger.info(f'start resume training at iteration: {iteration} ...')
            self.controller.clean_weight_files(iteration=0)
            logger.info('start to update resume weights to rollout unit before training...')
            self.update_weights_to_rollout_unit(False, iteration - 1)
        else:
            self.controller.clean_weight_files(iteration)

        logger.info(f'async start one step off grpo training at iteration: {iteration}/{self.train_iters} ...')
        ...
        while iteration < self.train_iters:
            ...
            json_save_data(batch, "train_batch_from_queue", iteration)
            ...
            with Timer(name='iteration', logger=None) as all_timer:
                ...
                logger.info('|perf-stat|train| compute_advantage start ...')
                # compute advantages, executed on the driver process
                self.compute_advantage(blocking=False, guarantee_order=self.guarantee_order)

                logger.info('|perf-stat|train| compute_ref_log_prob start ...')
                ...
                logger.info('|perf-stat|train| wait_all_ref_objs_run_over start ...')
                ...
                logger.info('|perf-stat|train| update start ...')
                # update actor
                self.actor_worker.update(self.kl_ctrl, self.skip_actor_log_prob)
                self.actor_worker.wait_all_ref_objs_run_over()
                logger.info('|perf-stat|train| update done')
            ...
            if iteration % self.save_interval == 0 or iteration == self.train_iters:
                logger.info('start saving checkpoint for iteration {iteration}...')
