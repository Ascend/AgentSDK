...
class OneStepOffRolloutExecutor:
    ...
    def fit(self):
        ...
        while iteration < self.train_iters and not ray.get(self.queue_actor.is_shutdown.remote()):
            ...
            # waiting until the train server to send two batch complete
            if queue_size < self.hybrid_batch_num and init_wait_times > 0:
                time.sleep(self.sleep_time)
                init_wait_times = init_wait_times - 1
                continue
            ...
            logger.info(
                f"available rollout queue size: {queue_size}, "
            ...
            logger.info(
                f"|perf-stat|rollout| ===rollout iteration for {actual_batch_num} batches: "
