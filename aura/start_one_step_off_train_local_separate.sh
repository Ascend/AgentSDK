...
# 建议环境变量配置同时修改runtime_env.yaml 可能export未生效
export WAIT_AVAILABLE_WEIGHT_TIMEOUT=-1 # rollout更新权重无限等待
export SCHEDULER_ALGO='lb-traj' # 适配pd混部
...
if [[ ${node_type} == "master" && ${task_type} == "train" ]]; then
  python aura/aura/start.py --config-name=one_step_off_16node_qwen3_235b_separate_train 2>&1 | tee ${logs_path}/train_logs_${timestamp}.log
  ray stop
elif [[ ${node_type} == "master" && ${task_type} == "rollout" ]]; then
  python aura/aura/start.py --config-name=one_step_off_32node_qwen3_235b_separate_rollout 2>&1 | tee ${logs_path}/rollout_logs_${timestamp}.log
  ray stop
fi
