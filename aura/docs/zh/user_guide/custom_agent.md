...
自定义 Agent 需要继承 [`BaseAgent`](../../../aura/runner/agent_engine_wrapper/base/agent/base_agent.py)，实现以下抽象方法：
...
> [!TIP] 参考
> 完整实现可参考内置 Agent：[`ToolAgent`](../../../agents/math_agent/tool_agent.py)

### 步骤二：实现自定义 Environment

自定义 Environment 需要继承 [`BaseEnv`](../../../aura/runner/agent_engine_wrapper/base/environment/base_env.py)，实现以下抽象方法：
...
> [!TIP] 参考
> 完整实现可参考内置 Environment：[`ToolEnvironment`](../../../agents/math_agent/environment/tool_env.py)

### 步骤三：注册到 agents_mapping

在 [`agents/agents_mapping.py`](../../../agents/agents_mapping.py) 的 `AGENTS_MAPPING` 列表中添加自定义 Agent 配置：
...
合并逻辑源码参见 [`rllm_engine_wrapper.py`](../../../aura/runner/agent_engine_wrapper/rllm/rllm_engine_wrapper.py)：
...
| Agent 名称 | 类 | 说明 |
|------------|-----|------|
| `math` | [`ToolAgent`](../../../agents/math_agent/tool_agent.py) | 数学推理 Agent，支持 Python 代码执行工具 |

## 源码实现原理

| 组件 | 源码位置 | 说明 |
|------|---------|------|
| Agent 注册表 | [`agents/agents_mapping.py`](../../../agents/agents_mapping.py) | 定义所有内置 Agent 配置，提供 `get_agent_by_name()` 查找 |
| Agent 基类 | [`base/agent/base_agent.py`](../../../aura/runner/agent_engine_wrapper/base/agent/base_agent.py) | `BaseAgent` 抽象类，定义 Agent 接口 |
| Environment 基类 | [`base/environment/base_env.py`](../../../aura/runner/agent_engine_wrapper/base/environment/base_env.py) | `BaseEnv` 抽象类，定义环境接口 |
| Agent 执行器 | [`agent_executor.py`](../../../aura/runner/agent_service/agent_executor.py) | `AgentExecutor`，创建 RLLMEngineWrapper |
| Agent 管理器 | [`agent_manager.py`](../../../aura/runner/agent_manager.py) | `AgentManager`，读取配置创建执行器实例 |
| Agent 路由 | [`agent_router.py`](../../../aura/runner/agent_router.py) | `AgentRouter`，路由请求到具体执行器 |
| RLLM 引擎 | [`rllm_engine_wrapper.py`](../../../aura/runner/agent_engine_wrapper/rllm/rllm_engine_wrapper.py) | `RLLMEngineWrapper`，本地 Agent 执行引擎 |
