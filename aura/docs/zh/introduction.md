...
- [训推共卡使用指南（On-Policy 策略）](user_guide/hybrid.md)：训练与推理在同一组卡上通过时分复用协同运行
- [训推分离使用指南（One-Step-Off 策略）](user_guide/one_step_off.md)：训练与推理在不同节点上并行执行
- [自定义 Agent 接入指南](user_guide/custom_agent.md)：将自定义 agent 接入 Aura 训练框架
