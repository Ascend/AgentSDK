...
# ---------------------------------------------------------------------------
# Fixture: prepare fake modules (torch handled separately)
# ---------------------------------------------------------------------------
@pytest.fixture
def fake_sampler_env():
    ...
    # ---- Fake vllm.config with LogprobsMode ----
    ...
    torch_where_mock = MagicMock(return_value=MagicMock())
...
# ---------------------------------------------------------------------------
def import_module(fake_sampler_env, env_getenv="False"):
    module_name = "aura.runner.infer_adapter.vllm.patch.patch_0_10_2.patch_vllm_sampler"
    ...
    fake_torch.where = fake_sampler_env["torch_where"]
    ...
    try:
        with patch.dict(sys.modules, fakes), \
            ...
            import aura.runner.infer_adapter.vllm.patch.patch_0_10_2.patch_vllm_sampler as mod
    finally:
...
# ---------------------------------------------------------------------------
def make_sampler_mock(logprobs_mode="raw_logprobs"):
    self = MagicMock()
...
class TestForwardPatch:
    ...
    def setup(self, fake_sampler_env):
        ...
        self.mod = import_module(fake_sampler_env, env_getenv="false")
        self.forward_patch = self.mod.forward_patch

    def test_num_logprobs_none(self):
        """num_logprobs=None skips gather_logprobs and returns SamplerOutput."""
    ...
    def test_raw_logprobs_mode(self):
        """num_logprobs>0 with RAW_LOGPROBS calls compute_logprobs."""
        self_mock = make_sampler_mock(logprobs_mode=self.env["LogprobsMode"].RAW_LOGPROBS)
        logits = MagicMock()
        sampling_metadata = MagicMock()
    ...
    def test_raw_logits_mode(self):
        """num_logprobs>0 with RAW_LOGITS clones logits instead of compute_logprobs."""
        self_mock = make_sampler_mock(logprobs_mode=self.env["LogprobsMode"].RAW_LOGITS)
        logits = MagicMock()
    ...
    def test_processed_logprobs_replaces_raw(self):
        """When sample returns processed_logprobs, raw is replaced."""
        self_mock = make_sampler_mock()
    ...
    def test_full_pipeline(self):
        """All steps covered: preprocess, penalize, sample, gather logprobs."""
        self_mock = make_sampler_mock()
        logits = MagicMock()
        sampling_metadata = MagicMock()
...
class TestSamplePatch:
    ...
    def test_both_greedy_and_random_raises(self):
        """Both all_greedy and all_random True raises ValueError."""
        self_mock = make_sampler_mock()
        logits = MagicMock()
        sm = MagicMock()
        sm.all_greedy = True
    ...
    def test_all_random(self):
        """all_random=True skips greedy, applies temperature and topk_topp."""
        self_mock = make_sampler_mock()
        logits = MagicMock()
        sm = MagicMock()
        sm.all_random = True
        ...
        self_mock.apply_temperature.assert_called_once_with(logits, sm.temperature)
        self_mock.topk_topp_sampler.assert_called_once()

    def test_all_greedy_no_logprobs(self):
        """all_greedy=True returns greedy samples, no processed_logprobs if max_num_logprobs is None."""
        self_mock = make_sampler_mock()
        logits = MagicMock()
        sm = MagicMock()
        sm.all_greedy = True
    ...
    def test_all_greedy_with_logprobs_mode(self):
        """all_greedy with max_num_logprobs returns logits/processed_logprobs based on mode."""
        self_mock = make_sampler_mock(logprobs_mode=self.env["LogprobsMode"].PROCESSED_LOGITS)
        logits = MagicMock()
        sm = MagicMock()
        sm.all_greedy = True
        sm.all_random = False
    ...
    def test_all_greedy_processed_logprobs_mode(self):
        """all_greedy with PROCESSED_LOGPROBS mode calls compute_logprobs."""
        self_mock = make_sampler_mock(logprobs_mode=self.env["LogprobsMode"].PROCESSED_LOGPROBS)
        logits = MagicMock()
        sm = MagicMock()
        sm.all_greedy = True
        sm.all_random = False
        ...
        self_mock.greedy_sample.assert_called_once()
        self_mock.compute_logprobs.assert_called_once_with(logits)
        assert probs is self_mock.compute_logprobs.return_value

    def test_temperature_none_raises(self):
        """Raises ValueError when temperature is None and not all_greedy."""
        self_mock = make_sampler_mock()
        logits = MagicMock()
        sm = MagicMock()
        sm.all_greedy = False
        ...
        sm.temperature = None
        sm.logitsprocs.argmax_invariant = []
        with pytest.raises(ValueError, match="temperature cannot be None for random sampling"):
            self.sample_patch(self_mock, logits, sm)

    def test_normal_path_with_greedy_sampled(self):
        """Normal path: greedy_sampled exists, temperature normal -> torch.where used."""
        self_mock = make_sampler_mock()
        ...
        torch_where = self.env["torch_where"]
        torch_where.assert_called_once()
        assert sampled is torch_where.return_value

    def test_greedy_none_returns_random(self):
        """When greedy_sampled is None, random_sampled is returned directly."""
        self_mock = make_sampler_mock()
...
class TestModulePatchCondition:
    def test_patch_applied_when_env_true(self, fake_sampler_env):
        """Sampler.forward/sample are patched when ENABLE_VLLM_STAT=true."""
        mod = import_module(fake_sampler_env, env_getenv="true")
    ...
    def test_patch_not_applied_when_env_false(self, fake_sampler_env):
        """Sampler methods remain original when ENABLE_VLLM_STAT=false."""
        mod = import_module(fake_sampler_env, env_getenv="false")
        Sampler = fake_sampler_env["Sampler"]
