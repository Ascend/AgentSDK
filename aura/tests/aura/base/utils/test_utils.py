...
class TestStrftime:
    ...
    @mock.patch('aura.base.utils.utils.datetime')
    def test_strftime_integer(self, mock_datetime):
        mock_datetime.datetime = mock.MagicMock(wraps=datetime.datetime)
        mock_datetime.datetime.fromtimestamp.return_value = datetime.datetime(2024, 8, 8, 9, 10, 45, 0)
        result = strftime(1723079445)
        assert result == "2024-08-08 09:10:45.000000"

    # Test with float timestamp
    @mock.patch('aura.base.utils.utils.datetime')
    def test_strftime_float(self, mock_datetime):
        mock_datetime.datetime = mock.MagicMock(wraps=datetime.datetime)
        mock_datetime.datetime.fromtimestamp.return_value = datetime.datetime(2024, 8, 8, 9, 10, 45, 123456)
        result = strftime(1723079445.123456)
