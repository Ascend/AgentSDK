...
if [ "$VC_TASK_INDEX" = "$MASTER_ROLLOUT_INDEX" ]; then
  ...
  sleep 1m
  ray status
  python aura/aura/start.py --config-name=one_step_off_qwen3_235b_separate_rollout 2>&1 | tee ${logs_path}/rollout_unit_${timestamp}.log
  # 结束ray集群
  ray stop
fi
...
if [ "$VC_TASK_INDEX" = "$MASTER_TRAIN_INDEX" ]; then
  ...
  sleep 1m
  ray status
  python aura/aura/start.py --config-name=one_step_off_qwen3_235b_separate_dummy_train 2>&1 | tee ${logs_path}/train_unit_${timestamp}.log
  # 结束ray集群
  ray stop
fi
