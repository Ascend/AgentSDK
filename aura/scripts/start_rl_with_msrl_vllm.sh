...
export RL_TRAIN_BACKEND="mindspeed_rl"
...
function start_train()
{
  ...
  log_info "start msrl train cluster, work_mode: ${WORK_MODE}, config name: ${real_train_conf_name}"
  if [[ "$1" == "daemon" ]]; then
    bash ${scripts_dir}/train/start_msrl_train_cluster.sh --config-name ${real_train_conf_name} | sed "s/^/[train_cluster] /" &
  else
    bash ${scripts_dir}/train/start_msrl_train_cluster.sh --config-name ${real_train_conf_name} | sed "s/^/[train_cluster] /"
  fi
}
...
function start_infer()
{
  ...
  if [[ "$1" == "daemon" ]]; then
    bash ${scripts_dir}/infer/start_vllm_infer_cluster.sh --config-name ${INFER_CONF_NAME} | sed "s/^/[infer_cluster] /" &
  else
    bash ${scripts_dir}/infer/start_vllm_infer_cluster.sh --config-name ${INFER_CONF_NAME} | sed "s/^/[infer_cluster] /"
  fi
}
...
function set_infer_visible_devices()
{
  ...
  local total_cards=$(npu-smi info -l | grep "Total Count" | awk '{print $4}')
