...
if [[ $1 == "master" && $4 == "train" ]]; then
  timestamp=$(date +%s%3N)
#  python aura/aura/start.py --config-name=one_step_off_1node_qwen25_7b_separate_dummy_train 2>&1 | tee ${logs_path}/logs_${timestamp}.log
  python aura/aura/start.py --config-name=one_step_off_qwq_32b_separate_dummy_train 2>&1 | tee ${logs_path}/logs_${timestamp}.log
  ray stop
elif [[ $1 == "master" && $4 == "rollout" ]]; then
  timestamp=$(date +%s%3N)
#  python aura/aura/start.py --config-name=one_step_off_1node_qwen25_7b_separate_rollout 2>&1 | tee ${logs_path}/logs_${timestamp}.log
  python aura/aura/start.py --config-name=one_step_off_qwq_32b_separate_rollout 2>&1 | tee ${logs_path}/logs_${timestamp}.log
  ray stop
fi
