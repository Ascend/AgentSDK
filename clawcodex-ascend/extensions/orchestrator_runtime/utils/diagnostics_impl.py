...
Deliberate copy so this package can detect process freezes without
importing the upstream diagnostics module. Later work may reshape
the class to the ``DiagnosticsProbe`` protocol (duck-typing on
...
The detector is **observation-only**: it never trips an abort
controller, never raises, and never modifies the agent loop.
Auto-recovery is a separate concern; this watchdog only gives
postmortem reporters (the ``diag freeze-report`` CLI) a dump to read.

A watchdog thread is used instead of ``asyncio.create_task`` because
the freeze can land while the only asyncio loop is blocked on I/O
(provider stream, NFS write). A daemon thread keeps ticking then.
"""
...
# Default poll cadence. 10 s is well under the 60 s default threshold
# (the detector must tick at least once within the threshold window)
# and far above the syscall cost of reading ``time.monotonic()``.
DEFAULT_FREEZE_CHECK_INTERVAL_S = 10.0

# ``CLAWCODEX_FREEZE_DIAG=1`` turns the watchdog on for an existing
# process. Empty/unset keeps it idle (no thread, no extra work).
...
class FreezeDump:
    ...
    def to_dict(self) -> dict[str, Any]:
        d = asdict(self)
        d["thread_stacks"] = [
            {"tid": f.tid, "thread_name": f.thread_name, "frames": f.frames} for f in self.thread_stacks
        ]
        return d
...
class FreezeDetector:
    ...
    def __init__(
        ...
        *,
        ...
        self._threshold = float(threshold) if threshold is not None else resolved.threshold_s
        self._check_interval = float(check_interval) if check_interval is not None else DEFAULT_FREEZE_CHECK_INTERVAL_S
        ...
        self._diag_env_enabled = bool(diag_env_enabled) if diag_env_enabled is not None else _diag_env_enabled()
    ...
    def _run(self) -> None:
        ...
        auto-recovery is handled elsewhere. Self-termination would
        mask the bug we are trying to diagnose.
        """
