...
"""Orchestrator-local copy of ``clawcodex_ext.intent_forecast.focus``.

Deliberate copy so workspace-focus detection works without importing
the upstream intent_forecast package. Later work may replace
``compute_workspace_focuses`` with a Protocol-based ``IntentFocus``.
"""
...
def suggestion_matches_focus(text: str, refs: list[str], focus: dict[str, Any]) -> bool:
    ...
    aliases = tuple(meta["strong_path_aliases"]) + tuple(meta["module_aliases"]) + tuple(meta["weak_text_aliases"])
