...
"""Orchestrator-local copy of ``clawcodex_ext.services.api.errors``.

Deliberate copy so retry logic can classify API errors without
importing the upstream services package. Unused helpers (for example
``is_media_size_error``) can be dropped once AgentRunner only needs
``RateLimitError`` and ``is_rate_limit_error``.
"""
