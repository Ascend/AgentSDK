...
class HybridTransport(WebSocketTransport):
    ...
    def __init__(
        ...
        *,
        ...
        #: Chain of in-flight fire-and-forget enqueues. Serializes
        #: stream-event batch enqueues so concurrent timer fires or
        #: writes cannot reorder batches at the uploader queue head.
    ...
    async def write(self, message: dict[str, Any]) -> None:
        ...
        await self._enqueue_ordered(self._enqueue_task, buffered + [message])
        await self._uploader.flush()

    async def write_batch(self, messages: list[dict[str, Any]]) -> None:
        buffered = self._take_stream_events()
        await self._enqueue_ordered(self._enqueue_task, buffered + list(messages))
        await self._uploader.flush()

    async def flush(self) -> None:
        buffered = self._take_stream_events()
        if buffered:
            await self._enqueue_ordered(self._enqueue_task, buffered)
        await self._uploader.flush()
    ...
    def close(self) -> None:
        ...
        leftover, self._stream_event_buffer = self._stream_event_buffer, []
        uploader = self._uploader
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(
                self._grace_close_uploader(uploader, leftover),
    ...
    def _on_stream_event_timer_fire(self) -> None:
        ...
        try:
            loop = asyncio.get_running_loop()
            prev = self._enqueue_task
            task = loop.create_task(
                self._enqueue_ordered(prev, buffered),
    ...
    async def _enqueue_ordered(
        self,
        prev_task: asyncio.Task[None] | None,
        events: list[dict[str, Any]],
    ) -> None:
        """Enqueue a batch, chained behind any in-flight enqueue.

        Timer fires are fire-and-forget (sync callback); without this
        chain, a timer batch overlapping an already-waiting enqueue can
        land at the uploader queue head ahead of it, reordering events.
        """
        if prev_task is not None:
            try:
                ...
                pass
        await self._uploader.enqueue(events)
    ...
    async def _grace_close_uploader(
        ...
        try:
            if leftover:
                await uploader.enqueue(leftover)
    ...
    async def _post_once(self, events: list[dict[str, Any]]) -> None:
        ...
        if not session_token:
            # Raise instead of returning: a silent return looks like a
            # successful send to the uploader and drops the batch without
            # any retry/backoff. Raising routes this into the uploader's
            # normal failure handling (backoff, then drop).
            logger.warning(
                "HybridTransport: no session token available for POST",
            )
            raise RuntimeError("no session-ingress token available for POST")
