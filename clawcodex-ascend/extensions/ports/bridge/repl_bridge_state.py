...
"""Internal state machine and helpers for the env-based REPL bridge.

Holds all mutable per-bridge state (``_BridgeState``) and the
``on_state_change`` dispatcher used by the initializer in
:mod:`extensions.ports.bridge.repl_bridge`.
"""
...
if TYPE_CHECKING:
    from extensions.ports.bridge.repl_bridge import (
        BridgeCoreParams,
        BridgeState,
        OnStateChange,
    )
...
class _BridgeState:
        ...
        **Strategy-2** (kill + create fresh session) otherwise.
        ...
        if new_env_id == prior_env_id and prior_session_id is not None and had_active_session:
            try:
                ...
                logger.info(
                    "[bridge:repl] Strategy-1 reconnect refused (session=%s): %s — falling back to Strategy-2",
                    prior_session_id,
                    err,
                )
            else:
    ...
    async def _process_work(self, work: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                "[bridge:repl] schedule refresh failed (likely undecodable JWT — child uses initial token): %s",
                err,
            )
    ...
    def send_result(self) -> bool:
        """Forward the final result to the remote consumer.

        ⚠️ NO-OP (MVP): always returns ``False``. The child emits its own
        result message when it finishes a turn; this method exists for API
        parity with ``remote_bridge_core`` and is wired in Phase 6 full.
        Callers must treat ``False`` as "result not submitted".
        """
        return False
