    ...
    if msg_type == "assistant":
        ...
        for block in content:
            ...
            if block_type == "tool_use":
                ...
                on_debug(f"[bridge:activity] sessionId={session_id} tool_use name={name} {_input_preview(tool_input)}")
        ...
        if subtype == "success":
            ...
            error_summary = (
                errors[0] if isinstance(errors, list) and errors and isinstance(errors[0], str) else f"Error: {subtype}"
            )
            activities.append(
                SessionActivity(
                    type="error",
...
class SessionSpawnerDeps:
    ...
    on_debug: Callable[[str], None] = logger.debug
    on_warning: Callable[[str], None] = logger.warning
...
class _SpawnedSession:
    ...
    def _send_kill_signal(self, *, force: bool) -> None:
        ...
        self._deps.on_debug(f"[bridge:session] Sending {sig_name} to sessionId={self._session_id} pid={process.pid}")
        try:
            ...
            self._deps.on_debug(f"[bridge:session] {sig_name.lower()} failed (already exited?): {err}")
    ...
    def update_access_token(self, token: str) -> None:
        ...
        self._deps.on_debug(f"[bridge:session] Sent token refresh via stdin for sessionId={self._session_id}")
    ...
    async def _spawn_and_wait_inner(self) -> None:
        ...
        if returncode in (-signal.SIGTERM, -signal.SIGINT):
            self._deps.on_debug(
                f"[bridge:session] sessionId={self._session_id} interrupted signal={-returncode} pid={process.pid}"
            )
            status = "interrupted"
        elif returncode == 0:
            self._deps.on_debug(
                f"[bridge:session] sessionId={self._session_id} completed exit_code=0 pid={process.pid}"
            )
            status = "completed"
        else:
            self._deps.on_debug(
                f"[bridge:session] sessionId={self._session_id} failed exit_code={returncode} pid={process.pid}"
            )
            status = "failed"
    ...
    async def _read_stdout(self) -> None:
        ...
        async for raw in self._iter_lines(self._process.stdout):
            ...
            self._deps.on_debug(f"[bridge:ws] sessionId={self._session_id} <<< {debug_truncate(line)}")
            ...
            for activity in extracted:
                ...
                if self._deps.on_activity is not None:
                    try:
                        ...
                        self._deps.on_warning(f"[bridge:session] on_activity raised: {err}")
        ...
        if msg_type == "control_request":
            ...
            if (
                ...
                try:
                    ...
                    self._deps.on_warning(f"[bridge:session] on_permission_request raised: {err}")
        elif (
            msg_type == "user"
            ...
            if text:
                ...
                try:
                    ...
                    self._deps.on_warning(f"[bridge:session] on_first_user_message raised: {err}")
    ...
    def _resolve_debug_file(self) -> str | None:
        ...
        if self._deps.debug_file is not None:
            ...
            if ext_idx > 0:
                return f"{self._deps.debug_file[:ext_idx]}-{safe_id}{self._deps.debug_file[ext_idx:]}"
