...
# Bounded history of completed work ids. ``deque(maxlen=...)`` evicts
# the oldest id once the cap is hit, so a long-running daemon does not
# grow memory unboundedly. The cap is a safety valve: the server
# processes stop_work promptly, so stale redeliveries beyond the window
# are rare.
MAX_COMPLETED_WORK_IDS = 1024
...
def parse_args(args: list[str]) -> ParsedArgs:
    ...
    * ``--help`` / ``-h``
    ...
    while i < len(args):
        ...
        if arg in ("--help", "-h"):
        ...
        elif arg == "--session-timeout" or arg.startswith("--session-timeout="):
            if arg.startswith("--session-timeout="):
                raw = arg[len("--session-timeout=") :]
            elif i + 1 < len(args):
                i += 1
                raw = args[i]
            else:
                raw = None
            st = _parse_session_timeout_value(raw)
            if isinstance(st, int):
                out.session_timeout_ms = st
            else:
                return _make_error(st)
            ...
            return _make_error(f"{arg.split('=')[0]} is a KAIROS-only flag not yet supported in the MVP")
...
def _parse_session_timeout_value(raw: str | None) -> int | str:
    """Parse ``--session-timeout`` seconds into ms.

    Returns an error message instead of raising, so ``parse_args`` can
    funnel invalid input through the same ``_make_error`` path as every
    other flag (mirrors ``_parse_capacity_value``). Non-positive values
    are rejected too — a 0/negative timeout would fire the watchdog
    immediately and kill the session right after spawn.
    """
    if raw is None:
        return "--session-timeout requires a positive integer (got: <missing>)"
    try:
        n = int(raw)
    except ValueError:
        return f"--session-timeout requires a positive integer (got: {raw!r})"
    if n < 1:
        return f"--session-timeout requires a positive integer (got: {raw!r})"
    return n * 1000
...
class _BridgeDaemon:
    ...
    def __init__(
        ...
        *,
        ...
        self.completed_work_ids: deque[str] = deque(maxlen=MAX_COMPLETED_WORK_IDS)
    ...
    async def run(self) -> None:
        ...
        at_cap_interval = cfg.poll_interval_ms_at_capacity / 1000.0 if cfg.poll_interval_ms_at_capacity > 0 else 60.0
        heartbeat_interval = (
            cfg.non_exclusive_heartbeat_interval_ms / 1000.0 if cfg.non_exclusive_heartbeat_interval_ms > 0 else None
        )
    ...
    async def _on_session_done(self, session_id: str) -> None:
        ...
        if work_id is not None:
            ...
            self.completed_work_ids.append(work_id)
