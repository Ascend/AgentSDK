...
from ._error_utils import operator_failure_detail
...
class OrchestratorRunMixin:
    def _start_issue_run(self, session: AgentSession) -> None:
        """Kick off :meth:`_run_issue` for a prepared session and track it.

        Shared by the issue-launch and review-follow-up launch paths so the
        gitignore sync, the status-dashboard event and the ``_tasks`` /
        ``_issue_tasks`` bookkeeping stay identical across mixins.
        """
        self._sync_gitignore_to_workspace(session.workspace)
        self.status_dashboard.on_session_start(
            SessionStatus(
                issue_id=session.issue.id or "",
                issue_identifier=session.issue.identifier or "",
        ...
        async with self._semaphore:
            ...
            try:
                ...
                try:
                    ...
                    await self._execute_session_agent(session, progress_sink)
                    await self._handle_session_completion(session)
                finally:
                ...
                await self._handle_git_sync_post_commit_error(session, exc)

            except VerificationFailed as exc:
                await self._handle_verification_failed(session, exc)

            except HookFailedError as exc:
                await self._handle_hook_failed(session, exc)

            except asyncio.TimeoutError:
                workspace_dirty = await self._handle_run_timeout(session)

            except asyncio.CancelledError:
                await self._handle_run_cancelled(session)

            except Exception as exc:
                await self._handle_run_failure(session, exc, ran_agent)
                ...
                await self._write_session_viz_events(session)
                await self._route_session_terminal_status(session)
    ...
    def _persist_session_report(self, session: AgentSession) -> None:
        """Snapshot the session report / end-state fields onto the registry record.

        Shared by the sync-success path and the post-commit failure handler so
        both persist the same report fields for a finished run.
        """
    ...
    async def _execute_session_agent(self, session: AgentSession, progress_sink: Any) -> None:
        """Run the repro gate (when applicable) and dispatch to the workflow / collaboration runner."""
    ...
    async def _handle_session_completion(self, session: AgentSession) -> None:
        """Post-run handling for terminal session statuses: cannot-proceed reporting, git_sync, review gate and salvage."""
        if session.status in (
            ...
            if session.status == "completed" and session.session_end_reason not in (
                ...
                if not _has_changes:
                    ...
                    session.session_end_summary = "Agent reported completed but workspace has no file changes"
            ...
            if sync_result is not None and sync_result.session_end_reason == "empty_branch_no_commits":
                ...
                session.session_end_summary = "Agent did not produce any file modifications; no PR was created."
            ...
            if sync_result is not None:
                self._persist_session_report(session)
    ...
    @staticmethod
    def _mark_verification_failed(session: AgentSession, output: str) -> None:
        """Record the shared ``verification_failed`` session state trio."""
        session.status = "verification_failed"
        session.verification_status = "failed"
        session.verification_output = output

    async def _handle_git_sync_post_commit_error(self, session: AgentSession, exc: GitSyncPostCommitError) -> None:
        """Record a GitSyncPostCommitError: register the commit/PR and mark the run verification-failed."""
        ...
        self._mark_verification_failed(session, exc.output)
        if exc.hook_name:
    ...
    async def _handle_verification_failed(self, session: AgentSession, exc: VerificationFailed) -> None:
        """Mark the session verification-failed after a VerificationFailed from git_sync."""
        logger.warning(
    ...
    async def _handle_hook_failed(self, session: AgentSession, exc: HookFailedError) -> None:
        """Mark the session verification-failed after a pre/post-commit hook failure."""
        logger.warning(
    ...
    async def _handle_run_timeout(self, session: AgentSession) -> bool:
        """Record an agent-run timeout and report whether the workspace was left dirty."""
    ...
    async def _handle_run_cancelled(self, session: AgentSession) -> None:
        """Record an operator-requested cancellation and clean up the workspace best-effort."""
    ...
    async def _handle_run_failure(self, session: AgentSession, exc: BaseException, ran_agent: bool) -> None:
        """Record an unexpected run failure with a concise operator-facing detail."""
        logger.exception(
        ...
        detail = operator_failure_detail(exc)
        ...
        session.operator_failure_detail = detail

    async def _write_session_viz_events(self, session: AgentSession) -> None:
        """Best-effort dashboard journal events for a finished run; never raises."""
    ...
    async def _route_session_terminal_status(self, session: AgentSession) -> None:
        """Apply the final registry/dashboard/tracker transition for the terminal session status."""
