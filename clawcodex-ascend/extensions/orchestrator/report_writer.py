...
from ._file_utils import write_text_utf8
...
def write(
    *,
    ...
    persistent_dir = Path.home() / ".clawcodex" / "reports" / safe_tracker / safe_owner / safe_repo / safe_issue_id
    workspace_dir.mkdir(parents=True, exist_ok=True)
    persistent_dir.mkdir(parents=True, exist_ok=True)

    workspace_md = workspace_dir / f"{safe_run_id}.md"
    workspace_json = workspace_dir / f"{safe_run_id}.json"
    persistent_md = persistent_dir / f"{safe_run_id}.md"
    persistent_json = persistent_dir / f"{safe_run_id}.json"

    markdown = _render_markdown(report)
    payload = json.dumps(asdict(report), indent=2, ensure_ascii=False)

    write_text_utf8(workspace_md, markdown)
    write_text_utf8(workspace_json, payload)
    ...
    if tool_events_path:
        ...
        if tool_events.exists():
            persistent_events = persistent_dir / f"{safe_run_id}.events.ndjson"
    ...
    if cast_path:
        ...
        if cast_src.exists():
            workspace_cast_file = workspace_dir / f"{safe_run_id}.cast"
            persistent_cast_file = persistent_dir / f"{safe_run_id}.cast"
            try:
