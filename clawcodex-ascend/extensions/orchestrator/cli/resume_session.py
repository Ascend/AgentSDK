...
def _render_summary(label: str, result) -> str:
    ...
    if result.metadata is not None:
        md = result.metadata
        lines.append(f"  model={md.model} cwd={md.cwd} title={md.title!r} messages={md.message_count}")
    else:
...
def _run_resume_session(
    ...
    if not result.success and not result.messages and not result.metadata:
        if session is None:
            print(
                f"error: no session found for run_id {target_run_id!r} and no transcript.jsonl is readable",
                file=sys.stderr,
            )
            return 1
