...
class CronRun:
    ...
    owner_agent_id: str | None = None  # agent that actually executes this run
    ...
    def from_dict(cls, data: dict[str, Any]) -> CronRun | None:
        try:
            ...
            if status not in ACTIVE_RUN_STATUSES and status not in TERMINAL_RUN_STATUSES:
                return None
            queued_at = int(_first_non_null(data, "queued_at", "queuedAt", "createdAt", "created_at") or 0)
            ...
            return cls(
                ...
                owner_agent_id=_optional_str(data.get("owner_agent_id", data.get("ownerAgentId"))),
                ended_at=ended_at,
            )
...
def write_cron_runs(workspace_root: Path, runs: Iterable[CronRun]) -> None:
    ...
    # Partition: all active runs are preserved unconditionally. Only
    # terminal (historical) runs are subject to the MAX_CRON_RUNS cap.
    # This prevents truncation from evicting an active run and breaking
    # the duplicate-run guard in create_queued_run.
    runs_list = list(runs)
    active = [r for r in runs_list if r.status in ACTIVE_RUN_STATUSES]
    terminal = [r for r in runs_list if r.status not in ACTIVE_RUN_STATUSES]
    terminal_sorted = sorted(terminal, key=lambda item: (item.queued_at, item.id), reverse=True)
    # Reserve capacity for active runs; cap terminal history to the remainder.
    terminal_cap = max(0, MAX_CRON_RUNS - len(active))
    kept = active + terminal_sorted[:terminal_cap]
    kept_sorted = sorted(kept, key=lambda item: (item.queued_at, item.id), reverse=True)
    payload = {
        "version": 2,
        "runs": [run.to_dict() for run in kept_sorted],
    }
...
def _first_non_null(data: dict[str, Any], *keys: str) -> Any:
    """Return the first non-None, non-empty value among ``keys`` in ``data``.

    Unlike ``_first_value`` which returns the first key whose value is not
    None, this also skips falsy-but-present values (e.g. ``0``) when they
    are followed by a later key that has a truthy value — matching the
    original ``or``-chain semantics for backward compat. Use
    ``_first_value`` when ``0`` is a valid value that should not be skipped.
    """
    for key in keys:
        value = data.get(key)
        if value:
