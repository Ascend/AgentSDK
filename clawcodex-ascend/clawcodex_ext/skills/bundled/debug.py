...
The full TS skill calls ``enableDebugLogging()`` to opt in mid-session.
Python has no equivalent helper today, so this port reports that limitation
instead of claiming the no-op enabled logging.
"""
...
def _enable_debug_logging() -> bool:
    """Attempt to enable logging and report whether that actually succeeded.

    The Python runtime currently exposes no mid-session logging toggle.
    Returning ``False`` keeps the prompt honest until a real implementation
    can replace this adapter.
    """
...
def _read_log_tail(log_path: str) -> str:
    ...
    try:
        ...
        tail = "\n".join(tail_lines)
        return f"Log size: {_format_file_size(size)}\n\n### Last {_DEFAULT_DEBUG_LINES_READ} lines\n\n```\n{tail}\n```"
    except FileNotFoundError:
        return "No debug log exists at the resolved path."
...
def _build_debug_prompt(args: str) -> str:
    logging_enabled = _enable_debug_logging()
    debug_log_path = _get_debug_log_path()
    log_info = _read_log_tail(debug_log_path)

    logging_status_section = (
        (f"\n## Debug Logging Enabled\n\nDebug logging is active at `{debug_log_path}`.\n")
        if logging_enabled
        else (
            "\n## Debug Logging Not Enabled\n\n"
            "No debug logging was enabled by this command. The Python runtime "
            "cannot enable it mid-session. Do not tell the user logging is "
            "active; an existing file may contain only earlier output. If a "
            "fresh capture is required, ask the user to restart with debug "
            "logging configured by the host runtime.\n"
        )
    )
...
def register_debug_skill() -> bool:
    return register_bundled_skill(
        BundledSkillDefinition(
            name="debug",
            description=("Inspect available session debug logs and help diagnose issues"),
