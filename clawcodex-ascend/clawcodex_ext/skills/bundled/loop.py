...
The visibility callback follows the runtime cron kill switch so ``/loop``
is not offered when recurring scheduling is disabled.
"""
...
def _is_loop_enabled() -> bool:
    """Follow the runtime cron feature gate, including the legacy spelling."""

    raw = os.environ.get("CLAWCODEX_DISABLE_CRON")
    if raw is None:
        raw = os.environ.get("CLAUDE_CODE_DISABLE_CRON")
    cron_disabled = raw is not None and raw.strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    }
    return not cron_disabled
...
def register_loop_skill() -> bool:
    return register_bundled_skill(
        BundledSkillDefinition(
            name="loop",
            description=(
                "Run a prompt on a fixed interval, including default 10m prompt loops and bare maintenance-mode loops."
            ),
            when_to_use=(
