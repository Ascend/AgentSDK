...
def _coerce_description(raw: Any, markdown_content: str, resolved_name: str) -> tuple[str, bool]:
    ...
    if raw is None or raw == "":
        body_desc = _extract_description_from_markdown(markdown_content, default=f"Skill: {resolved_name}")
...
def _coerce_model(value: Any) -> str | None:
    ...
    if lower not in known_aliases and not looks_canonical:
        logger.warning(
            "skill frontmatter model %r is not a recognized alias or canonical model name; keeping as-is",
            s,
        )
    return s
...
def _coerce_hooks(value: Any, *, skill_name: str) -> dict | None:
    ...
    try:
        from clawcodex_ext.hooks.hook_types import ALL_HOOK_EVENTS  # pylint: disable=no-name-in-module
...
def parse_skill_frontmatter_fields(
    ...
    description, has_user_specified_description = _coerce_description(raw_desc, markdown_content, resolved_name)
...
def load_skills_from_skills_dir(
    ...
    *,
    ...
    """
    resolved_loaded_from = loaded_from if loaded_from is not None else _SOURCE_TO_LOADED_FROM.get(source, "skills")
...
def _is_path_gitignored(path: str, cwd: str) -> bool:
    ...
    try:
        result = subprocess.run(
            [git_executable, "check-ignore", path],
            cwd=cwd,
            capture_output=True,
            text=True,
            encoding="utf-8",
...
def activate_conditional_skills_for_paths(
    ...
    activated: list[str] = []
    workspace_dynamic = _dynamic_skills_by_workspace.setdefault(_workspace_key(cwd), {})
    ...
    if activated:
        _invalidate_catalog_cache_only(cwd)
    return activated
...
def _invalidate_catalog_cache_only(workspace: str | Path | None = None) -> None:
    try:
        catalog = importlib.import_module("clawcodex_ext.skills.catalog")
    except ImportError:
        return
...
from .model import PromptSkill  # noqa: E402, F401  (re-exported for back-compat)
...
def discover_all_skills(
    *,
    ...
    if bundle is not None:
        return [skill for skill in get_dynamic_skills(project_root) if skill.name in bundle.skill_names]
    ...
    disk_other = [skill for skill in disk_skills if skill.loaded_from not in {"managed", "user", "project"}]
    merge_order = (
...
def get_registered_skill(name: str) -> Skill | None:
    ...
    try:
        ...
        logger.debug("Failed to load builtin plugin skill commands", exc_info=True)
    return None
