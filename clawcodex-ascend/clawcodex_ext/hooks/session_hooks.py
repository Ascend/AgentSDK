...
class UserPromptSubmitOutcome:
    ...
    __slots__ = ("additional_contexts", "block_message", "blocked", "prevent_reason", "prevented")
    ...
    *,
    ...
    for config in configs:
        ...
        if getattr(result, "prevent_continuation", False):
            ...
            outcome.prevent_reason = getattr(result, "stop_reason", None) or "blocked by hook"
            break
        ...
        if extra:
            outcome.additional_contexts.extend(str(c)[:MAX_HOOK_OUTPUT_LENGTH] for c in extra)
