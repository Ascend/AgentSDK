...
def _get_hooks_from_snapshot(tool_use_context: Any) -> dict[str, list[HookConfig]]:  # pylint: disable=function-redefined
...
_IF_CONDITION_EVENTS = frozenset({"PreToolUse", "PostToolUse", "PostToolUseFailure", "PermissionRequest"})
...
_IF_FILE_PATH_TOOLS = frozenset({"Read", "Edit", "MultiEdit", "Write", "NotebookEdit"})
...
def _match_wildcard_pattern(pattern: str, value: str) -> bool:
    ...
    regex_pattern = with_wildcards.replace(_ESCAPED_STAR, "\\*").replace(_ESCAPED_BACKSLASH, "\\\\")
...
def _matchable_values_for_tool(tool_name: str, tool_input: dict[str, Any]) -> list[str] | None:
...
def _matches_if_condition(
    ...
    * rule-content → matched with matchWildcardPattern against the tool's
    ...
    matcher = _command_rule_matches if current in _IF_COMMAND_TOOLS else _match_wildcard_pattern
...
async def _execute_command_hook(
    ...
    try:
        ...
        if stdout:  # pylint: disable=too-many-nested-blocks
            ...
            if err is not None:
                ...
                if isinstance(hso, dict) and hso.get("hookEventName") == "PreToolUse":
                    ...
                    if pd is not None:
                        if pd in ("allow", "deny", "ask"):
                            ...
                            if pd == "deny":
                                result.hook_permission_decision_reason = (
                                    hso.get("permissionDecisionReason") or parsed.reason or "Blocked by hook"
                                )
                        else:
                ...
                if isinstance(hso, dict):
                    ...
                    if isinstance(_hso_ac, str) and _hso_ac:
                        result.additional_contexts = list(result.additional_contexts or []) + [_hso_ac]
...
async def _run_hooks_for_event(
    ...
    for hook in event_hooks:
        ...
        if not _matches_if_condition(getattr(hook, "if_condition", None), event, tool_name, tool_input):
            continue
        ...
        if result.exit_code is not None and result.exit_code != 0 and result.exit_code != 2:
            ...
            if hook.once and hook.source == HookSource.SKILL:
                ...
                if isinstance(registered, dict):
                    registered[event] = [candidate for candidate in registered.get(event, []) if candidate is not hook]
...
def _parse_elicitation_hook_output(
    ...
    if action == "decline":
        default = (
            "Elicitation denied by hook" if expected_event == "Elicitation" else "Elicitation result blocked by hook"
        )
...
async def execute_elicitation_hooks(
    ...
    *,
    ...
    return await _run_elicitation_event_hooks("Elicitation", server_name, stdin_data, tool_use_context)
...
async def execute_elicitation_result_hooks(
    ...
    *,
    ...
    return await _run_elicitation_event_hooks("ElicitationResult", server_name, stdin_data, tool_use_context)
        ...
        for hook in event_hooks:
            try:
                ...
                await _execute_command_hook(
                    hook,
                    {**stdin_data, "hook_event": "Notification"},
                    abort_signal=abort_signal,
                    tool_use_context=tool_use_context,
                )
            except Exception:
