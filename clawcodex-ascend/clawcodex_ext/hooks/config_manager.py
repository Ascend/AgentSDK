...
from .hook_types import ALL_HOOK_EVENTS, HookConfig, HookEvent, HookSource
...
parse_hook_config = _parse_hook_config
                ...
                if shell_raw is not None and shell_raw not in SHELL_TYPES:
                    errors.append(
                        HookValidationError(
                            ...
                            index=i,
                            field="shell",
                            message=(f"Unknown shell type: {shell_raw!r}. Must be one of {SHELL_TYPES}."),
                        )
                    )
            elif hook_type == "http":
...
def load_hooks_from_settings(
    ...
    *,
    ...
    return load_hooks_from_mapping(data, source_path=path, source=source)
        ...
        for path, src in scope_files:
            ...
            for event_name, hook_configs in snap.hooks.items():
                for config in hook_configs:
                    ...
                    if src is HookSource.USER_SETTINGS and event_name in ALL_HOOK_EVENTS:
...
def bootstrap_hook_config_manager(
    *,
    ...
    """
    try:
        from ..settings.settings import load_settings  # pylint: disable=relative-beyond-top-level

        if not load_settings(cwd=cwd).hooks.enabled:
            logger.info("hooks disabled via settings.hooks.enabled")
            return None
    except Exception:
        logger.debug("could not read settings.hooks.enabled; assuming enabled", exc_info=True)
    ...
    try:
        ...
        logger.warning("hook config bootstrap failed; continuing without hooks", exc_info=True)
        return None
