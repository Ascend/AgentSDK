...
def content_block_from_dict(data: Mapping[str, Any]) -> ContentBlock:
    ...
    if block_type == "tool_result":
        ...
        if isinstance(raw_content, list):
            content: str | list[Any] = [
                content_block_to_dict(item) if _is_content_block_instance(item) else item for item in raw_content
            ]
...
def content_block_to_dict(block: Any) -> dict[str, Any]:
    ...
    if isinstance(block, ToolResultBlock):
        ...
        if isinstance(block.content, list):
            result_content = [
                content_block_to_dict(item) if _is_content_block_instance(item) else item for item in block.content
            ]
        else:
