...
TypedMessage: TypeAlias = UserMessage | AssistantMessage | SystemMessage | ProgressMessage | AttachmentMessage | Message
...
def is_tool_use_request_message(message: Message) -> bool:
    ...
    if isinstance(content, list):
        return any(
            getattr(block, "type", None) == "tool_use" or (isinstance(block, dict) and block.get("type") == "tool_use")
            for block in content
        )
    return False
