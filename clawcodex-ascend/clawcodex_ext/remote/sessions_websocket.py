...
class SessionsWebSocket:
    ...
    async def connect(self) -> None:
        ...
        url = f"{self._base_url}/v1/sessions/ws/{self._session_id}/subscribe?organization_uuid={self._org_uuid}"
        headers = {
        ...
        if self._state != "connecting":
            logger.debug(
                "[SessionsWebSocket] disconnect() ran during handshake (state=%s); closing orphan WS",
                self._state,
            )
            try:
                await ws.close()
