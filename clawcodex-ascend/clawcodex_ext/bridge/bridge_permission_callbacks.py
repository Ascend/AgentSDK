...
Originally lived at ``src/bridge/bridge_permission_callbacks.py``; moved
to ``clawcodex_ext/`` per the Decoupling Mandate. The ``src/`` shim is a
``sys.modules`` facade so
``from src.bridge.bridge_permission_callbacks import ...`` continues to
work for the small number of legacy callers in ``extensions/`` and tests.
"""
