...
class _BridgeApiClient:
    ...
    async def register_bridge_environment(self, config: BridgeConfig) -> dict[str, str]:
        ...
        self._debug(f"[bridge:api] POST /v1/environments/bridge -> {response.status_code} environment_id={env_id}")
    ...
    async def poll_for_work(
        ...
        params: dict[str, Any] | None = (
            {"reclaim_older_than_ms": reclaim_older_than_ms} if reclaim_older_than_ms is not None else None
        )
        ...
        if data is None:
            ...
            if self._consecutive_empty_polls == 1 or self._consecutive_empty_polls % self._EMPTY_POLL_LOG_INTERVAL == 0:
                self._debug(
        ...
        self._debug(
            ...
            f"workId={data.get('id')} type={inner_type}" + (f" sessionId={inner_id}" if inner_id else "")
        )
        self._debug(f"[bridge:api] <<< {debug_body(data)}")
...
def _handle_error_status(
    ...
    if status == 429:
        raise Exception(f"{context}: Rate limited (429). Polling too frequently.")  # pylint: disable=broad-exception-raised
    raise Exception(f"{context}: Failed with status {status}" + (f": {detail}" if detail else ""))  # pylint: disable=broad-exception-raised
