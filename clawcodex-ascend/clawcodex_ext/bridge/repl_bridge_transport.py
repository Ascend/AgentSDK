...
P3-out-2: real implementation moved from ``src/bridge/repl_bridge_transport.py``.
Import chain adjusted to point at the Layer 1 / Layer 2 real modules
directly (``clawcodex_ext.transports.*`` and
``extensions.ports.transports.hybrid_v1``), skipping the
``src.transports.*`` thin forwarding seams. The legacy src path is now
itself a ``from clawcodex_ext.bridge.repl_bridge_transport import *``
facade.
"""
...
from clawcodex_ext.transports.ccr_client import CCRClient, CCRClientOptions
from clawcodex_ext.transports.sse_transport import SSEEvent, SSETransport
from extensions.ports.transports.hybrid_v1 import HybridTransport
...
class _V2ReplTransport:
    ...
    def set_on_close(self, cb: Callable[[int | None], None]) -> None:
        ...
        self._sse.set_on_close(cb)  # pylint: disable=unnecessary-lambda
