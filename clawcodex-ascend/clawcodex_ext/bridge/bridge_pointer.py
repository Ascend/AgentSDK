...
def read_pointer(
    ...
    *,
    ...
    if data.get("schema_version") != _SCHEMA_VERSION:
        logger.info(
            "[bridge:pointer] schema_version mismatch in %s (file=%r, expected=%r) — treating as absent",
            path,
    ...
    try:
        ...
        logger.info(
            "[bridge:pointer] %s missing or malformed required fields (%s) — treating as absent",
            path,
            err,
        )
        return None
    ...
    if file_machine != machine_name:
        logger.info(
            "[bridge:pointer] %s machine mismatch (file=%r, current=%r) — treating as absent",
            path,
            file_machine,
            machine_name,
        )
    ...
    if os.path.abspath(file_dir) != os.path.abspath(working_dir):
        logger.info(
            "[bridge:pointer] %s dir mismatch (file=%r, current=%r) — treating as absent",
            path,
            file_dir,
            working_dir,
        )
...
def clear_pointer(working_dir: str) -> None:
    ...
    try:
        ...
        logger.warning(
            "[bridge:pointer] unlink(%s) failed: %s",
            path,
            err,
        )
