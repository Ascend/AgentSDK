                ...
                if behavior == "allow":
                    ...
                    yield {
                        ...
                        "hookPermissionResult": {
                            "behavior": "ask",
                            "updatedInput": result.get("updated_input"),
                            "message": result.get("hook_permission_decision_reason")
                            or f"Hook PreToolUse:{tool.name} requires approval",
                            "decisionReason": decision_reason,
                        },
                    }
                else:
                    yield {
                        ...
                        "hookPermissionResult": {
                            "behavior": behavior,
                            "message": result.get("hook_permission_decision_reason")
                            or f"Hook PreToolUse:{tool.name} denied this tool",
                            "decisionReason": decision_reason,
                        },
                    }
            ...
            try:
                ...
                if hasattr(full_check, "behavior"):
                    if full_check.behavior == "deny":
                        return {
                            "behavior": "deny",
                            "message": getattr(full_check, "message", "Permission denied"),
                        }
        ...
        if can_use_tool is not None and callable(can_use_tool):
            try:
                ...
                if inspect.iscoroutinefunction(can_use_tool):
                    decision = await can_use_tool(tool, tool_input, tool_use_context, assistant_message, tool_use_id)
                else:
                    decision = can_use_tool(tool, tool_input, tool_use_context, assistant_message, tool_use_id)
                ...
                return {
                    "behavior": "deny",
                    "message": (f"Permission handler returned an unrecognized decision for {tool.name}"),
                }
            except Exception as e:
    ...
    if behavior == "allow":
        ...
        if (requires_interaction and not interaction_satisfied) or require_can_use_tool:
            if can_use_tool is not None and callable(can_use_tool):
                ...
                if inspect.iscoroutinefunction(can_use_tool):
                    decision = await can_use_tool(tool, hook_input, tool_use_context, assistant_message, tool_use_id)
                else:
                    decision = can_use_tool(tool, hook_input, tool_use_context, assistant_message, tool_use_id)
                if isinstance(decision, dict):
