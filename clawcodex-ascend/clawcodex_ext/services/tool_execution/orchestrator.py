...
# Re-use the MessageUpdate / ToolUseBlock dataclasses from streaming_executor
# so external callers that imported them from the old orchestrator module
# keep working.
...
async def _run_tools_serially(
    ...
    for tool_use in tool_use_messages:
        if current_context.set_in_progress_tool_use_ids:
            current_context.set_in_progress_tool_use_ids(lambda prev, tu=tool_use: prev | {tu.id})
    ...
    for msg in assistant_messages:
        ...
        if isinstance(content, list):
            for block in content:
                if isinstance(block, dict) and block.get("type") == "tool_use" and block.get("id") == tool_use.id:
                    return msg
