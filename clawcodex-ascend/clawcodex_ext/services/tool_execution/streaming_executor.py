...
class StreamingToolExecutor:
    def __init__(
        ...
        self._sibling_abort_controller = create_child_abort_controller(tool_use_context.abort_controller)
    ...
    def add_tool(self, block: ToolUseBlock, assistant_message: AssistantMessage) -> None:
        ...
        from .orchestrator import classify_concurrency_safe
    ...
    def _update_interruptible_state(self) -> None:
        ...
        if setter is None:
            return
        executing = [t for t in self._tools if t.status == "executing"]
        interruptible = len(executing) > 0 and all(self._get_tool_interrupt_behavior(t) == "cancel" for t in executing)
        try:
            ...
            logger.warning("set_has_interruptible_tool_in_progress raised: %s", exc)

    async def _execute_tool(self, tool: TrackedTool) -> None:
        tool.status = "executing"
        if self._tool_use_context.set_in_progress_tool_use_ids:
            self._tool_use_context.set_in_progress_tool_use_ids(lambda prev: prev | {tool.id})
        ...
        async def collect_results() -> None:
            ...
            if initial_abort:
                messages.append(self._create_synthetic_error_message(tool.id, initial_abort, tool.assistant_message))
            ...
            def _on_tool_abort() -> None:
                ...
                if (
                    ...
                    self._tool_use_context.abort_controller.abort(tool_abort_controller.signal.reason)
    ...
    async def get_remaining_results(self) -> AsyncGenerator[MessageUpdate, None]:
        ...
        while self._has_unfinished_tools():
            ...
            executing_promises = [t.promise for t in self._tools if t.status == "executing" and t.promise is not None]

            if not executing_promises:
                continue

            progress_wait_task = asyncio.ensure_future(self._progress_available_event.wait())
            try:
                await asyncio.wait(
...
def _mark_tool_use_as_complete(tool_use_context: ToolContext, tool_use_id: str) -> None:
    if tool_use_context.set_in_progress_tool_use_ids:
        tool_use_context.set_in_progress_tool_use_ids(lambda prev: prev - {tool_use_id})
