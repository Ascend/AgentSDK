...
PersistResult = PersistedToolResult | PersistToolResultError
    ...
    if is_json:
        ...
        for block in content:  # type: ignore[union-attr]
            ...
            if block_type not in ("text",):
                return PersistToolResultError(error="Cannot persist tool results containing non-text content")
...
def build_large_tool_result_message(result: PersistedToolResult) -> str:
    ...
    parts.append(
        f"Output too large ({_format_file_size(result.original_size)}). Full output saved to: {result.filepath}"
    )
...
def resolve_tool_results_dir(context: ToolContext) -> Path:
    ...
    if session_id and workspace_root is not None:
        ...
        return Path.home() / ".clawcodex" / workspace_basename / str(session_id) / TOOL_RESULTS_SUBDIR
    return Path("/tmp") / "clawcodex_tool_results" / str(os.getpid()) / TOOL_RESULTS_SUBDIR  # nosec B108
