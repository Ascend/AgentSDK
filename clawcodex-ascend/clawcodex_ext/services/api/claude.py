...
def add_cache_breakpoints(
    ...
    *,
    ...
    if isinstance(content, str):
        new_content: list[dict[str, Any]] = [{"type": "text", "text": content, "cache_control": cache_control}]
    ...
    try:
        if client is None:
            try:
                ...
                client = anthropic.AsyncAnthropic(default_headers=_headers or None)
            except ImportError:
        ...
        try:
            from clawcodex_ext.utils.image_validation import ImageSizeError, validate_images_for_api  # pylint: disable=no-name-in-module
