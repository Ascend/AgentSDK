...
from clawcodex_ext.services.session_storage import SessionMetadata, SessionStorage
...
def _rewrite_content_list(blocks: list, old_cwd: str, new_cwd: str) -> None:  # pylint: disable=too-many-nested-blocks
...
def _check_chain_consistency(messages: list[Message]) -> list[str]:
    ...
    for i, msg in enumerate(messages):  # pylint: disable=too-many-nested-blocks
        ...
        if prev_role == role:
            warnings.append(f"Message {i}: consecutive '{role}' messages (prev was message {i - 1})")

        # Check ordering: user → assistant → user → assistant
        if prev_role == "user" and role == "user":
            warnings.append(f"Message {i}: two consecutive user messages; expected assistant between them")
        if prev_role == "assistant" and role == "assistant":
            warnings.append(f"Message {i}: two consecutive assistant messages; expected user between them")
            ...
            if not content:
                ...
                if not has_text and not has_tool_use:
                    ...
                    if not isinstance(content, str) or not content.strip():
                        warnings.append(f"Message {i}: assistant message has no text or tool_use content")
