...
def _get_global_config_dir() -> Path:
    env_override = os.environ.get("CLAWCODEX_CONFIG_DIR") or os.environ.get("CLAUDE_CONFIG_DIR")
    if env_override:
...
def _add_scope_to_servers(
    ...
    if not servers:
        return {}
    return {name: ScopedMcpServerConfig(config=config, scope=scope) for name, config in servers.items()}
...
def unwrap_ccr_proxy_url(url: str) -> str:
    ...
    try:
        ...
        if len(wrapped) > 1:
            logger.warning(
                "MCP CCR-proxy unwrap: %d 'mcp_url' params present; refusing to guess. Keeping original URL: %s",
                len(wrapped),
                url,
            )
            return url
...
def _expand_env_vars(
    ...
    if isinstance(config, McpStdioServerConfig):
        ...
        expanded_headers = {k: _expand(v) for k, v in config.headers.items()} if config.headers else None
...
def add_dynamic_mcp_config(name: str, config: McpServerConfig) -> None:
    ...
    with _dynamic_mcp_lock:
        _dynamic_mcp_configs[name] = ScopedMcpServerConfig(config=config, scope="dynamic")
...
def get_all_mcp_configs() -> tuple[dict[str, ScopedMcpServerConfig], list[ValidationError]]:
    ...
    if claudeai_servers:
        ...
        manual_only_servers = {k: merged[k] for k in manual_keys if k in merged and merged[k].scope != "claudeai"}
...
def _suggestion_for_message(message: str) -> str | None:
    ...
    if "is required" in message:
        ...
        field_name = message.split(" is required", 1)[0].strip()
        return f"Add a `{field_name}` field to the server config"
    if "cannot be empty" in message:
        field_name = message.split(" cannot be empty", 1)[0].strip()
        return f"Set `{field_name}` to a non-empty string"
    ...
    if scope == "project":
        ...
        if config_file.exists():
            ...
            if not isinstance(data, dict):
                raise ValueError(f"user config {config_file} is not a JSON object")
...
def get_claude_desktop_config_path() -> Path:
    ...
    if system == "Darwin":
        ...
        return (
            ...
            if appdata
            else Path.home() / "Claude" / "claude_desktop_config.json"
        )
    else:
...
def filter_mcp_servers_by_policy(
    ...
    if _policy_flag("disable_all_mcp", "disableAllMcp"):
        if configs:
            notices.append("All MCP servers disabled by policy (settings.disable_all_mcp).")
        return {}, notices

    if _policy_flag("allow_managed_only_mcp", "allowManagedOnlyMcp"):
        kept = {name: cfg for name, cfg in configs.items() if cfg.scope in ("enterprise", "managed")}
