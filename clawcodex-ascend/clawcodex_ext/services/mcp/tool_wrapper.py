...
# Sibling clawcodex_ext packages (permissions) are migrated in separate branches;
...
def _get_input_validator(server_name: str, tool_name: str, schema: dict[str, Any]) -> Optional[Any]:
    ...
    schema_hash = hashlib.sha1(schema_blob, usedforsecurity=False).hexdigest()[:16]
    ...
    try:
        ...
        logger.warning(
            "MCP %s/%s: failed to compile input schema validator (%s); falling back to passthrough validation",
            server_name,
            tool_name,
            exc,
        )
