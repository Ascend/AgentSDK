...
class McpClient:
            ...
            if init_result and isinstance(init_result, dict):
                ...
                if raw_instructions and isinstance(raw_instructions, str):
                    if len(raw_instructions) > MAX_MCP_DESCRIPTION_LENGTH:
                        self._instructions = raw_instructions[:MAX_MCP_DESCRIPTION_LENGTH] + "... [truncated]"
                    else:
    ...
    def _create_transport(self, config: McpServerConfig, *, server_name: str | None = None) -> McpTransport:
        ...
        if isinstance(config, (McpSSEServerConfig, McpHTTPServerConfig, McpWebSocketServerConfig)):
            ...
            if self._auth_provider is not None and server_name is not None and "Authorization" not in headers:
        ...
        *,
        ...
        async with self._recovery_lock:
            if self._session_generation != gen_at_entry:
                ...
                logger.info(
                    "MCP %r tool %r: session expired (gen %d); piggybacking on concurrent reconnect (now gen %d)",
                    self._name,
                    tool_name,
                    gen_at_entry,
