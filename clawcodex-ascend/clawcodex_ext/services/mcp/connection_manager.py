    ...
    async def reconnect_mcp_server(self, name: str) -> MCPServerConnection:
        ...
        async with self._lock_for(name):
            ...
            try:
                ...
                client, conn = await connect_to_server(name, config, auth_provider=self._auth_provider)
    ...
    *,
    ...
    if prefetch_claudeai:
        try:
            ...
            logger.warning(
                "MCP bootstrap: claudeai prefetch failed (%s); continuing without web-configured connectors.",
                exc,
            )
