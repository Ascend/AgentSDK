    ...
    *,
    ...
    try:
        ...
        if response.status_code != 200:
            raise RuntimeError(f"OIDC discovery returned HTTP {response.status_code} for {idp_issuer}")
        try:
...
async def acquire_idp_id_token(
    ...
    *,
    ...
    if s is None:
        raise RuntimeError(
            "XAA IdP login is disabled or unconfigured (ENABLE_MCP_XAA / MCP_XAA_ISSUER / MCP_XAA_CLIENT_ID)"
        )
    ...
    try:
        ...
        try:
            ...
            raise OAuthCallbackError(f"XAA IdP login timed out after {IDP_LOGIN_TIMEOUT_S:.0f}s") from exc
        ...
        if response.status_code != 200:
            raise RuntimeError(f"XAA IdP token exchange returned HTTP {response.status_code}: {response.text[:200]}")
        try:
            body = response.json()
        except ValueError as exc:
