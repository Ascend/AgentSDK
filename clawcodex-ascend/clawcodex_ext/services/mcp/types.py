...
def validate_server_config(data: Any) -> tuple[McpServerConfig | None, list[str]]:
    ...
    if server_type in ("sse", "http", "ws"):
        url = _validate_string_field(data.get("url"), "url", errors, required=True, allow_empty=False)
    ...
    if server_type == "sdk":
        name = _validate_string_field(data.get("name"), "name", errors, required=True, allow_empty=False)
        if errors or name is None:
            return None, errors
        return McpSdkServerConfig(name=name), errors

    # stdio (default branch — type is "stdio" or omitted)
    command = _validate_string_field(data.get("command"), "command", errors, required=True, allow_empty=False)
