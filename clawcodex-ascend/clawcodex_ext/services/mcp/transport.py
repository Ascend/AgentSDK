...
def _lazy_sse():
    ...
    if _MCP_CLIENT_SSE is None:
        from mcp.client.sse import sse_client  # pylint: disable=no-name-in-module
...
def _lazy_stdio():
    ...
    if _MCP_CLIENT_STDIO is None:
        from mcp.client.stdio import StdioServerParameters, stdio_client  # pylint: disable=no-name-in-module
...
def _lazy_streamable_http():
    ...
    if _MCP_CLIENT_STREAMABLE_HTTP is None:
        from mcp.client.streamable_http import streamable_http_client  # pylint: disable=no-name-in-module
...
def _lazy_websocket():
    ...
    if _MCP_CLIENT_WEBSOCKET is None:
        from mcp.client.websocket import websocket_client  # pylint: disable=no-name-in-module
...
def _lazy_shared():
    ...
    if _MCP_SHARED_MESSAGE is None:
        from mcp.shared.message import SessionMessage  # pylint: disable=no-name-in-module
...
def _lazy_types():
    ...
    if _MCP_TYPES_JSONRPC is None:
        from mcp.types import JSONRPCMessage  # pylint: disable=no-name-in-module
