...
async def wait_for_callback(
    ...
    *,
        ...
        try:
            ...
            while True:
                ...
                if not line or line in (b"\r\n", b"\n"):
                    break
            await _process_request(request_line, expected_state, path, result_future, writer)
            ...
            except Exception:  # pragma: no cover - socket teardown variance  # nosec B110 - best-effort socket teardown
                pass
    ...
    try:
        ...
        raise OAuthCallbackError(f"OAuth callback timed out after {timeout:.0f}s") from exc
    finally:
        server.close()
        try:
            await server.wait_closed()
        except Exception:  # pragma: no cover  # nosec B110 - best-effort server teardown
            pass
...
async def _process_request(
    ...
    if len(parts) < 2 or parts[0] != "GET":
        _write_response(
            ...
            405,
            "Method Not Allowed",
            _error_body("Only GET is supported on the callback endpoint."),
        )
        if not result_future.done():
...
def _write_response(writer: asyncio.StreamWriter, status: int, reason: str, body: str) -> None:
