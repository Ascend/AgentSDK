...
def find_available_port(env_override_name: str = "MCP_OAUTH_CALLBACK_PORT") -> int:
    ...
    candidates = random.sample(_port_range(), _MAX_ATTEMPTS)  # nosec B311 - port pick is not security-sensitive
    for port in candidates:
        if _is_port_free(port):
            return port
    logger.warning(
        "OAuth port allocator exhausted %d attempts in range %d-%d; falling back to %d (may already be bound)",
        _MAX_ATTEMPTS,
        _port_range().start,
        _port_range().stop - 1,
        _FALLBACK_PORT,
