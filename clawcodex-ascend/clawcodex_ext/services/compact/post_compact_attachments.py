...
from clawcodex_ext.types.content_blocks import ToolResultBlock, ToolUseBlock  # pylint: disable=no-name-in-module
from clawcodex_ext.types.messages import Message, UserMessage  # pylint: disable=no-name-in-module
...
def _should_exclude_from_post_compact_restore(
    ...
    if memory_paths and normalized in memory_paths:
        return True

    basename = os.path.basename(filename).lower()
    if basename in ("clawcodex.md", ".clawcodex.md"):
        return True
    return bool(basename.endswith((".clawcodex.md", "clawcodex.md")))
...
def create_post_compact_file_attachments(
    ...
    preserved_read_paths = _collect_read_tool_file_paths(preserved_messages) if preserved_messages else set()
