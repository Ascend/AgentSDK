...
from clawcodex_ext.types.content_blocks import TextBlock, ToolResultBlock, ToolUseBlock  # pylint: disable=no-name-in-module
...
def _has_tool_use_with_ids(message: Message, tool_use_ids: set[str]) -> bool:
    ...
    for block in content:
        if isinstance(block, ToolUseBlock) and block.id in tool_use_ids:
            return True
        if isinstance(block, dict) and block.get("type") == "tool_use" and block.get("id", "") in tool_use_ids:
            return True
    return False
...
def adjust_index_to_preserve_api_invariants(
    ...
    if all_tool_result_ids:  # pylint: disable=too-many-nested-blocks
