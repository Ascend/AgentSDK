...
from clawcodex_ext.types.content_blocks import ContentBlock, TextBlock, ToolResultBlock  # pylint: disable=no-name-in-module
from clawcodex_ext.types.messages import Message, UserMessage  # pylint: disable=no-name-in-module
from clawcodex_ext.utils.token_estimation import count_tokens  # pylint: disable=no-name-in-module
...
class BudgetManifest:
    ...
    def save(self, budget_dir: Path) -> None:
        ...
        data = [
            {"tool_use_id": s.tool_use_id, "path": s.path, "original_tokens": s.original_tokens} for s in self.stored
        ]
...
def apply_tool_result_budget(
    ...
    if budget_dir is None:
        budget_dir = Path("/tmp/claw_codex_budget") / str(os.getpid())  # nosec B108
