...
from clawcodex_ext.types.content_blocks import TextBlock, ToolUseBlock  # pylint: disable=no-name-in-module
from clawcodex_ext.types.messages import (  # pylint: disable=no-name-in-module
    Message,
    UserMessage,
    normalize_messages_for_api,
)
from clawcodex_ext.utils.token_estimation import (  # pylint: disable=no-name-in-module
    count_messages_tokens,
    rough_token_count_estimation_for_messages,
)

from .compact_warning import clear_compact_warning_suppression, suppress_compact_warning
...
def _record_compaction_usage(context: CompactContext, usage: Any) -> None:
    ...
    try:
        ...
        model = (
            getattr(context, "model", None) or getattr(getattr(context, "provider", None), "model", None) or "unknown"
        )
...
async def compact_conversation(
        ...
        try:
            ...
            if _is_prompt_too_long_error(error_str):
                if attempt < MAX_PTL_RETRIES:
                    ...
                    truncated = truncate_head_for_ptl_retry(messages_to_compact, token_gap=token_gap)
    ...
    user_display = (
        f"Compacted conversation (~{tokens_saved:,} tokens saved). Pre-compact: {pre_compact_tokens:,} tokens."
    )
...
async def partial_compact_conversation(
    ...
    if direction in ("up_to", "earlier"):
        anchor_uuid = getattr(summary_msg, "uuid", None) or getattr(boundary_msg, "uuid", None) or ""
    else:
        anchor_uuid = getattr(boundary_msg, "uuid", None) or ""

    if anchor_uuid:
        boundary_msg = annotate_boundary_with_preserved_segment(boundary_msg, anchor_uuid, list(messages_to_keep))
