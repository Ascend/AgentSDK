...
from clawcodex_ext.services.swarm.leader_permission_bridge import (
    deliver_permission_decision,
)
from clawcodex_ext.services.swarm.mailbox import get_inbox_path, read_mailbox
...
def _looks_like_teammate_task_state(prev: Any) -> bool:
    """Duck-typed check: does ``prev`` carry the teammate-state fields
    this module reads?

    Replaces the previous ``isinstance(prev, InProcessTeammateTaskState)``
    check so this module stays free of any ``src.tasks.*`` import. The
    real ``InProcessTeammateTaskState`` has all four attributes, so this
    is functionally equivalent for any teammate state produced by the
    in-process teammate task layer.
    """
    return (
        hasattr(prev, "shutdown_requested")
        and hasattr(prev, "awaiting_plan_approval")
        and hasattr(prev, "permission_mode")
        and hasattr(prev, "pending_user_messages")
    )
...
def _dispatch_plan_approval_response(
    *,
    ...
    def _apply(prev: Any) -> Any:
        if not _looks_like_teammate_task_state(prev):
            return prev
        new_permission_mode = permission_mode if isinstance(permission_mode, str) else prev.permission_mode
        return replace(
            prev,
