...
def _sanitize_name(name: str, *, kind: str) -> str:
    ...
    if not isinstance(name, str) or not _VALID_NAME_RE.match(name):
        raise ValueError(f"Invalid {kind}: {name!r} (allowed chars: A-Z, a-z, 0-9, _, -; max 64; min 1).")
    return name
            ...
            for raw_line in handle:
                ...
                try:
                    ...
                    if not logged_partial:
                        logger.warning(
                            "skipping unparseable mailbox line in %s (file may have a trailing partial-write)",
                            path,
                        )
                        logged_partial = True
                    continue
...
def make_iso_timestamp() -> str:
    ...
    return _dt.datetime.fromtimestamp(time.time(), tz=_dt.timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%fZ")


def create_shutdown_request_message(*, request_id: str, from_: str, reason: str | None = None) -> dict[str, Any]:
