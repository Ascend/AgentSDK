...
The "bones + soul" split is load-bearing: bones are regenerated on every
read from ``hash(user_id + SALT)``; the soul is the only thing that persists.
This means species renames and edits to ``SPECIES`` never break stored companions,
and editing ``config['companion']`` can't fake a rarity.

Divergence from TS: ``companion_user_id()`` reads ``config['user_id']`` only
...
from clawcodex_ext.buddy.types import (
    EYES,
    HATS,
    RARITIES,
    RARITY_WEIGHTS,
...
def _roll_from(rng: Callable[[], float]) -> Roll:
    """Roll companion bones from a seeded RNG."""
...
def roll_with_seed(seed: str) -> Roll:
    """Roll from a caller-provided seed (no cache)."""
...
def companion_user_id() -> str:
    """ID source for the deterministic bones roll."""
