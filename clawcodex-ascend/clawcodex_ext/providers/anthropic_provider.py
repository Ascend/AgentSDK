...
class AnthropicProvider(BaseProvider):
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None, model: Optional[str] = None):
        ...
        if self._subscription_token is not None:
            ...
            self._client_kwargs["default_headers"] = subscription_headers(self._client_kwargs.get("default_headers"))
        self.client = None
    ...
    def _prepare_messages(self, messages: list[Any]) -> list[dict[str, Any]]:
        ...
        from .openai_responses import strip_responses_item_blocks  # pylint: disable=no-name-in-module
    ...
    def _effective_base_url(self) -> str | None:
        ...
        """
        return self._client_kwargs.get("base_url") or os.environ.get("ANTHROPIC_BASE_URL") or None
    ...
    def _build_chat_response(self, response: Any) -> ChatResponse:
        ...
        for block in response.content:
            ...
            if block_type in ("thinking", "redacted_thinking"):
                ...
                if callable(dump):
                    thinking_blocks.append(_strip_none_deep(dict(dump(exclude_none=True))))
            ...
            if is_advisor:
                ...
                if callable(dump):
                    ...
                    raw_content_blocks.append({k: v for k, v in vars(block).items() if v is not None})
                continue
            if block_type == "text":
    ...
    def _client_for_request(self, kwargs: dict[str, Any]):
        ...
        if sdk_max_retries is not None:
            try:
                ...
                logger.debug(
                    "with_options(max_retries=%s) failed; SDK default retries remain active under the retry lane",
                    sdk_max_retries,
                    exc_info=True,
                )
                return client
    ...
    def chat(
        ...
        anthropic_messages, tools, system = self._prepare_subscription_request(anthropic_messages, tools, system)
    ...
    def chat_stream_response(  # pylint: disable=arguments-renamed
        self,
        ...
        **kwargs,
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if attempt < max_attempts:
                logger.warning(
                    "stream idle watchdog fired (attempt %d/%d, x-client-request-id=%s); retrying stream",
                    attempt,
                    max_attempts,
                    request_id or "n/a",
                )
    ...
    def _stream_attempt(
        ...
        *,
        ...
        try:
            ...
            if watchdog_fired:
                logger.debug(
                    "stream attempt interrupted by idle watchdog (x-client-request-id=%s): %s",
                    request_id or "n/a",
                    streaming_exc,
                )
                return None
