...
#
# Derived from NousResearch/hermes-agent's agent/moonshot_schema.py:
# https://github.com/NousResearch/hermes-agent
# Modified for AgentSDK. This file remains licensed under the MIT License.
...
Known rejection modes (documented in the Moonshot community forum):
...
This module is ported from hermes-agent's ``agent/moonshot_schema.py`` and
adapted for the clawcodex Layer-1 provider extension layout.
"""
...
def _repair_schema(node: Any, is_schema: bool = True) -> Any:
    ...
    for key, value in node.items():
        if key in _SCHEMA_MAP_KEYS and isinstance(value, dict):
            ...
            repaired[key] = {sub_key: _repair_schema(sub_val, is_schema=True) for sub_key, sub_val in value.items()}
