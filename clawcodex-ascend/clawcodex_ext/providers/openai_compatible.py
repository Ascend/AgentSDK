...
def _convert_anthropic_messages_to_openai(  # pylint: disable=too-many-nested-blocks
    messages: list[dict[str, Any]],
    *,
        ...
        if role == "user":
            ...
            for tr in tool_results:
                ...
                if tool_use_id not in known_tool_call_ids:
                    logger.debug(
                        "Dropping orphan tool_result for tool_use_id=%r (no matching tool_call in history)",
                        tool_use_id,
                    )
                    continue
                ...
                if multimodal_blocks_from_tool:
                    correlation = (
                        f"[multimodal content for tool_use_id={tool_use_id} delivered in the following message]"
                    )
                    if flat_content:
...
# Public aliases keep cross-module callers on the supported provider surface.
convert_anthropic_messages_to_openai = _convert_anthropic_messages_to_openai
convert_to_openai_tool_schema = _convert_to_openai_tool_schema
...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def chat_stream_response(  # pylint: disable=arguments-renamed
        ...
        **kwargs,
        ...
        disable_usage = os.environ.get("CLAWCODEX_DISABLE_STREAM_USAGE", "").lower() in ("1", "true", "yes")
