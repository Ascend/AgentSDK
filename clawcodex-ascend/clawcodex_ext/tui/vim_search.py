...
def _find_forward(buffer: VimBuffer, rx: re.Pattern[str], after: Cursor, *, wrap: bool) -> SearchHit | None:
...
def _find_backward(buffer: VimBuffer, rx: re.Pattern[str], after: Cursor, *, wrap: bool) -> SearchHit | None:
    n_lines = buffer.line_count
    if n_lines == 0:
        return None
...
class VimSearchState:
    ...
    def repeat(
        ...
        *,
        ...
        if reverse:
            direction = SearchDirection.BACKWARD if direction is SearchDirection.FORWARD else SearchDirection.FORWARD
        return find_next(
            buffer,
