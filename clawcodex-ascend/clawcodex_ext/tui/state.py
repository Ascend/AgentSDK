...
class PendingPermission:
    ...
    ``decide`` is called from the permission modal with the legacy
    ``(allowed, enable_setting)`` pair expected by ``ToolContext``; it
    is safe to invoke from either thread. ``suggestion`` is the optional
    human-readable rule shown by the modal.
    """
...
class AppState:
    ...
    def resolve_permission(self, request_id: str) -> None:
        with self._lock:
            self.pending_permissions = [p for p in self.pending_permissions if p.request_id != request_id]
        self.recompute_focus()
        self._notify()
    ...
    def resolve_ask_user(self, request_id: str) -> None:
        with self._lock:
            self.pending_ask_users = [p for p in self.pending_ask_users if p.request_id != request_id]
        self.recompute_focus()
        self._notify()
