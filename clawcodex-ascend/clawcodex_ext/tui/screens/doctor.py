...
class DoctorScreen(ModalScreen[None]):
    ...
    def _render_config_health(self) -> Any:
        ...
        try:
            ...
            warnings = collect_config_warnings(str(self._workspace_root) if self._workspace_root else None)
        except Exception:
    ...
    def _render_rule_health(self) -> Any:
        ...
        try:
            ...
            warnings = collect_rule_warnings(str(self._workspace_root) if self._workspace_root else None)
        except Exception:
            return Text("check failed", style="dim")
        if not warnings:
    ...
    def _render_environment(self) -> Any:
        ...
        def kv(label: str, value: Any) -> None:
            rows.append(Text(f"{label}: ", style="dim").append(str(value) if value else "(unknown)", style="default"))
        ...
        try:
            ...
            if callable(list_fn):
                try:
                    ...
                    count = None
            rows.append(Text("module: ", style="dim").append("src.services.session_storage", style="default"))
            if count is None:
                rows.append(
                    Text(
