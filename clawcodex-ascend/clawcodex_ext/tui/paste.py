...
def detect_image_drag(text: str) -> bool:
    ...
    for token in _PATH_SPLIT_RE.split(stripped):
        for raw_line in token.split("\n"):
            ...
            if not candidate or not (candidate.startswith("/") or _looks_like_windows_path(candidate)):
                continue
            dot = candidate.rfind(".")
...
def _looks_like_windows_path(candidate: str) -> bool:
    ...
    return len(candidate) >= 3 and candidate[0].isalpha() and candidate[1] == ":" and candidate[2] == "\\"
