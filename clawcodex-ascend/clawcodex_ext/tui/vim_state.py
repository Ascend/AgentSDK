...
def _from_idle(key: str, ctx: TransitionContext) -> TransitionResult:
    ...
    if key in (";", ","):
        ...
        if key == ",":
            ...
            find_type = flipped
        return TransitionResult(execute=_find_executor(ctx, find_type, char, count=1, update_last_find=False))
...
def _from_operator(op: Operator, count: int, key: str, ctx: TransitionContext) -> TransitionResult:
    ...
    if key in _TEXT_OBJ_SCOPES:
        return TransitionResult(next=OperatorTextObjState(op=op, count=count, scope=_TEXT_OBJ_SCOPES[key]))
...
def _from_operator_count(op: Operator, count: int, digits: str, key: str, ctx: TransitionContext) -> TransitionResult:
    ...
    if key in _TEXT_OBJ_SCOPES:
        return TransitionResult(next=OperatorTextObjState(op=op, count=effective, scope=_TEXT_OBJ_SCOPES[key]))
...
def _from_find(find_type: FindType, count: int, key: str, ctx: TransitionContext) -> TransitionResult:
...
def _from_operator_g(op: Operator, count: int, key: str, ctx: TransitionContext) -> TransitionResult:
...
def _from_indent(direction: Literal[">", "<"], count: int, key: str, ctx: TransitionContext) -> TransitionResult:
...
def _operator_motion_executor(ctx: TransitionContext, op: Operator, motion: str, *, count: int) -> Callable[[], None]:
    def go() -> None:
...
def _indent_executor(ctx: TransitionContext, direction: Literal[">", "<"], *, count: int) -> Callable[[], None]:
    def go() -> None:
        from .vim_buffer import Cursor, Range, VimBuffer
...
def _operator_gg_executor(ctx: TransitionContext, op: Operator, *, count: int) -> Callable[[], None]:
