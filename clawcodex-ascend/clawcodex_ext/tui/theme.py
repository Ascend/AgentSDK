...
def resolve_auto_theme(*, env: dict[str, str] | None = None) -> str:
    ...
    defaults_command = shutil.which("defaults")
    if defaults_command and (environment.get("__CFBundleIdentifier") or environment.get("TERM_PROGRAM")):
        try:
            out = subprocess.run(
                [defaults_command, "read", "-g", "AppleInterfaceStyle"],
                capture_output=True,
                text=True,
                timeout=0.5,
                check=False,
