...
def _owned_in_progress_tasks(
    ...
    *,
    ...
    if not agent_id:
        return []
    identities = {identity for identity in (agent_id, teammate_name) if isinstance(identity, str) and identity}
    try:
        task_map = getattr(tool_use_context, "tasks", {}) or {}
        return [
            task
            for task in list(task_map.values())
            if isinstance(task, dict) and task.get("status") == "in_progress" and task.get("owner") in identities
        ]
...
def _owner_exit_review_message(tasks: list[dict[str, Any]]) -> str:
    shown = tasks[:10]
    task_lines = [f"- {task.get('id', '<unknown>')}: {task.get('subject') or '<untitled>'}" for task in shown]
...
async def _handle_stop_hooks_generator(  # pylint: disable=too-many-nested-blocks
    messages_for_query: list[Message],
    assistant_messages: list[AssistantMessage],
    system_prompt: str,
    tool_use_context: Any,
        ...
        if core_gate:
            async for hook_result in execute_stop_hooks(
                ...
                if hook_result.get("blocking_error"):
                    ...
                    has_output = True
                    hook_errors.append(
                        error_info.get("blocking_error", "") if isinstance(error_info, dict) else str(error_info)
                    )
            ...
            async def _drive(generator, event_name: str, message_prefix: str):
                ...
                async for result in generator:
                    ...
                    if result.get("blocking_error"):
                        ...
                        text = error_info.get("blocking_error", "") if isinstance(error_info, dict) else str(error_info)
                        user_msg = create_user_message(
                    ...
                    if result.get("prevent_continuation"):
                        ...
                        teammate_stop_reason = result.get("stop_reason") or f"{event_name} hook prevented continuation"
                        yield create_attachment_message(
                            {
                                "type": "hook_stopped_continuation",
            ...
            if has_hook_for_event("TaskCompleted", tool_use_context):
                try:
                    ...
                    owned = [
                        ...
                        if isinstance(t, dict) and t.get("status") == "in_progress" and t.get("owner") == teammate_name
                    ]
...
def _get_permission_mode(tool_use_context: Any) -> str | None:
    try:
        ...
        logger.debug("Failed to read permission mode from app state", exc_info=True)
