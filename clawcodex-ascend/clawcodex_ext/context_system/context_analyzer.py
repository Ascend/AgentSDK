...
from clawcodex_ext.utils.token_estimation import count_tokens, count_messages_tokens  # pylint: disable=no-name-in-module
...
def get_context_window_for_model(model: str) -> int:
    ...
    try:
        ...
        logger.debug("Canonical model context lookup failed", exc_info=True)
...
def format_context_as_markdown(data: ContextData) -> str:
    ...
    if data.is_auto_compact_enabled:
        lines.append("**Auto-compact:** enabled")
    lines.append("")
