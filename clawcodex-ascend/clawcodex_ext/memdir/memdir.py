...
def _how_to_save_section(skip_index: bool) -> list[str]:
    ...
    return [
        ...
        *MEMORY_FRONTMATTER_EXAMPLE,
        ...
        "",
        f"- `{ENTRYPOINT_NAME}` is always loaded into your conversation context — it is truncated after {MAX_ENTRYPOINT_LINES} lines or {format_file_size(MAX_ENTRYPOINT_BYTES)}, whichever comes first, so keep the index concise (one short line per entry; long or many entries lose the tail)",
        "- Keep the name, description, and type fields in memory files up-to-date with the content",
        "- Organize memory semantically by topic, not chronologically",
        "- Update or remove memories that turn out to be wrong or outdated",
        "- Do not write duplicate memories. First check if there is an existing memory you can update before writing a new one.",
...
def load_memory_prompts(
    ...
    Args:
        memory_scopes: List of scopes to load. Supported scopes:
            ...
            - "local": Local/project-level memories

    Returns a list of memory prompt strings. Empty list if auto-memory
    is disabled or no prompts could be loaded for the specified scopes.
    """
