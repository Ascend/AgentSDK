        ...
        try:
            ...
            if code in (errno.ENOENT, errno.ENOTDIR):
                ...
                if os.path.islink(current):
                    raise PathTraversalError(f'Dangling symlink detected (target does not exist): "{current}"') from exc
                ...
                raise PathTraversalError(f'Cannot verify path containment ({code}): "{current}"') from exc
