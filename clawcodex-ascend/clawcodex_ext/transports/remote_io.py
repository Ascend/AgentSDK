...
Originally lived at ``src/transports/remote_io.py``; moved to
``clawcodex_ext/`` per the Decoupling Mandate. The ``src/`` shim is a
``sys.modules`` facade so ``from src.transports.remote_io import ...``
    ...
    def __init__(
        ...
        *,
        ...
        if initial_prompt is not None:
            self._initial_prompt_task = loop.create_task(self._consume_initial_prompt(initial_prompt))
    ...
    async def _iter_input(self) -> AsyncIterator[str]:
        while True:
            ...
            if not isinstance(item, str):
                raise TypeError(f"RemoteIO input queue produced non-str non-sentinel: {type(item).__name__}")
            yield item
