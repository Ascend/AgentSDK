...
Originally lived at ``src/transports/worker_state_uploader.py``; moved to
``clawcodex_ext/transports/worker_state_uploader.py`` per the Decoupling
Mandate. The ``src/`` shim is a ``sys.modules`` facade so legacy imports
continue to resolve.
"""
...
class WorkerStateUploader:
    ...
    def enqueue(self, patch: dict[str, Any]) -> None:
        ...
        if self._closed:
            return
        self._pending = _coalesce_patches(self._pending, patch) if self._pending is not None else dict(patch)
    ...
    def _retry_delay(self, failures: int) -> float:
        ...
        return exponential + random.random() * self._config.jitter_ms  # nosec B311 — jitter, not security
