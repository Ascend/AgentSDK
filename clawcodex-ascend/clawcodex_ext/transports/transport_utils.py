...
Originally lived at ``src/transports/transport_utils.py``; moved to
``clawcodex_ext/`` per the Decoupling Mandate. The ``src/`` shim is a
``sys.modules`` facade so ``from src.transports.transport_utils import
...`` continues to work for the small number of legacy callers in src.
The three sibling-transport imports below remain ``from src.transports``
per Golden Rule #2 (Layer 1 may import Layer 0).
"""
