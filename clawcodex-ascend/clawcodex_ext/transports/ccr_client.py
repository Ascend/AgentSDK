...
Originally lived at ``src/transports/ccr_client.py``; moved to
``clawcodex_ext/transports/ccr_client.py`` per the Decoupling Mandate.
The ``src/`` shim is a ``sys.modules`` facade so legacy imports continue
to resolve.
"""
...
class CCRClient:
    ...
    def __init__(
        ...
        self._queue: asyncio.Queue[dict[str, Any]] = asyncio.Queue(maxsize=self._options.queue_max_size)
    ...
    async def aclose(self) -> None:
        ...
        for task in (self._uploader_task, self._heartbeat_task):
            ...
            try:
                ...
                pass  # These tasks were explicitly cancelled; awaiting them only drains shutdown cleanup.
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        while not self._closed:
            try:
                ...
                delta = random.uniform(-jitter, jitter) * interval if jitter > 0 else 0.0  # nosec B311 — jitter, not security
