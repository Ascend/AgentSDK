...
def buddy_command_call(args: str, context: CommandContext) -> LocalCommandResult:
    ...
    if arg in COMMON_INFO_ARGS:
        ...
        return LocalCommandResult(
            type="text",
            value=(
                f"{companion.name} is your {_title_case(companion.rarity)} {companion.species}. {companion.personality}"
            ),
        )
    ...
    if companion is None:
        ...
        return LocalCommandResult(
            type="text",
            value=(f"{companion.name} the {companion.species} is now your buddy. Run /buddy again to pet them."),
        )
