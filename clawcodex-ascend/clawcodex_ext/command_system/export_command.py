...
from clawcodex_ext.utils.export_formats import (  # pylint: disable=no-name-in-module
    ExportFormat,
        ...
        *,
        ...
        try:
            ...
            with open(filepath, "w", encoding="utf-8") as fh:
                fh.write(content)
            return InteractiveOutcome(message=f"Conversation exported to: {filepath}", display="system")
        except Exception as exc:  # mirror TS catch-all (export.tsx:79)
            return InteractiveOutcome(message=f"Failed to export conversation: {exc}", display="system")
