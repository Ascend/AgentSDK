...
def _extract_file_mentions(text: str, cwd: str | None = None) -> list[str]:
    ...
    for match in _FILE_MENTION_RE.finditer(text):
        ...
        if not path_str:
            continue
        looks_like_path = path_str.startswith(("/", "~", "./", "../")) or "/" in path_str or "." in path_str
...
def _binary_hint_for_ext(ext: str) -> str:
    ...
    return (
        "This file is binary and cannot be inlined as text. Use the Read tool to view it if it is a supported format."
    )
...
def format_at_mention_attachments(attachments: list[dict[str, Any]]) -> str:
    ...
    for att in attachments:
        ...
        if kind == "directory":
            blocks.append(
                f"<system-reminder>\nListed directory {att['display_path']}/:\n{att['content']}\n</system-reminder>"
            )
        elif kind == "file":
            blocks.append(
                f"<system-reminder>\nContents of {att['display_path']}:\n```\n{att['content']}\n```\n</system-reminder>"
            )
        elif kind == "image":
