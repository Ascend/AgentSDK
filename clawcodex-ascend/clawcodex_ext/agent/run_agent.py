...
async def run_agent(params: RunAgentParams) -> AsyncGenerator[Message, None]:
    ...
    """
    from clawcodex_ext.query.query import QueryParams, StreamEvent, query
    ...
    # Build system prompt
    system_prompt = params.system_prompt_override or get_agent_system_prompt(agent_def, params.parent_system_prompt)
    ...
    try:
        ...
        if resolved_model and resolved_model != getattr(params.provider, "model", None):
            turn_provider = copy.copy(params.provider)
            turn_provider.model = resolved_model
    except Exception:  # noqa: BLE001  # nosec — model resolution never blocks a spawn
        logger.debug("subagent model resolution failed; using session model", exc_info=True)
        turn_provider = params.provider
    ...
    try:
        ...
        except Exception:  # noqa: BLE001  # nosec — cleanup must not break agent exit
