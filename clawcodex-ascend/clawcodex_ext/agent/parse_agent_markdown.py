...
AGENT_COLORS: frozenset[str] = frozenset({"red", "blue", "green", "yellow", "purple", "orange", "pink", "cyan"})
...
def parse_agent_from_markdown(
    ...
    permission_mode = parse_permission_mode(_first(frontmatter, "permission-mode", "permissionMode"))
    ...
    required_mcp_servers = parse_string_list(_first(frontmatter, "required-mcp-servers", "requiredMcpServers"))
    mcp_servers_raw = _first(frontmatter, "mcp-servers", "mcpServers")
    mcp_servers: list[Any] | None = list(mcp_servers_raw) if isinstance(mcp_servers_raw, list) else None
