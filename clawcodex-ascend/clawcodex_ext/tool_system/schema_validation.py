...
def format_tool_validation_error(tool_name: str, issues: list[ValidationIssue]) -> str:
    ...
    parts += [f"An unexpected parameter `{i.param}` was provided (unexpected field)" for i in unexpected]
    parts += [
    ...
    prefixed = "; ".join(
        f"{tool_name}.{i.path}: {i.message}" if i.path else f"{tool_name}: {i.message}" for i in issues[:5]
    )
    if len(issues) > 5:
...
def _validate(value: Any, schema: Mapping[str, Any], *, path: str, issues: list[ValidationIssue]) -> None:
            ...
            if isinstance(item_schema, dict):
                for idx, item in enumerate(value):
                    _validate(
                        item,
                        item_schema,
                        path=_child_index_path(path, idx),
                        issues=issues,
                    )
            return
    ...
    if "const" in schema:
        ...
        if value != expected:
            issues.append(ValidationIssue(path, f"expected const {expected!r}, got {value!r}"))
            return
