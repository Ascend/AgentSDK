...
async def _route_in_process(*, to: str, message_text: str, context: ToolContext) -> ToolResult | None:
    ...
    if not is_terminal_task_status(state.status):
        ...
        if not queue_pending_message(agent_id, message_text, context.runtime_tasks):
            return _err(f"Failed to queue message for {to!r} (task may have transitioned to terminal).")
        return _ok(
...
def _approve_flag(message_obj: dict[str, Any]) -> bool:
    ...
    if not isinstance(coerced, bool):
        raise ToolInputError('approve must be a boolean (or the string literal "true"/"false").')
    return coerced
...
def _structured_message_to_envelope(
    *,
    ...
    raise ToolInputError(
        f"Unknown structured-protocol type: {msg_type!r}. Expected one of: {sorted(_VALID_STRUCTURED_TYPES)}."
    )
...
async def _send_message_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    if isinstance(raw_message, str):
        ...
        # Branch 5 — not found.
        return _err(
            f"Recipient {to!r} not found (no in-process agent with that name/id; no team context to mailbox into)."
        )
    ...
    team_file = read_team_file(context.workspace_root)
    if team_file is None:
        raise ToolInputError("structured-protocol messages require a team file; TeamCreate hasn't run.")
