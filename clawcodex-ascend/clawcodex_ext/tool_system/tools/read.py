...
def _parse_pdf_pages(pages: str) -> tuple[int | None, int | None]:
    ...
    if not isinstance(pages, str) or not pages.strip():
        raise ToolInputError(f'Invalid pages parameter: "{pages}". Use formats like "1-5", "3", or "10-20".')
    s = pages.strip()
    try:
        if "-" in s:
        ...
        raise ToolInputError(f'Invalid pages parameter: "{pages}". Use formats like "1-5", "3", or "10-20".') from e
    if first < 1 or last < first:
        raise ToolInputError(f'Invalid pages range "{pages}": pages are 1-indexed and last must be >= first.')
...
def _get_dedup_fingerprint(
    fp_entry: tuple[int, ...] | tuple[int, int, bool],
) -> tuple[int, int]:
...
def _read_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    if _is_blocked_device_path(file_path):
        raise ToolInputError(f"Cannot read '{file_path}': this device file would block or produce infinite output.")
    ...
    if not path.is_file():
        ...
        if path.is_dir():
            raise ToolInputError(f"EISDIR: illegal operation on a directory, read '{path}'")
