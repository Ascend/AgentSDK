...
def spawn_background_bash(
    *,
    ...
    output_handle = Path(output_path).open("wb", buffering=0)  # noqa: SIM115
    ...
    def _reap() -> None:
        try:
            ...
            def _patch(prev: Any) -> Any:
                ...
                new_status = (
                    "killed" if getattr(prev, "status", None) == "killed" else ("completed" if rc == 0 else "failed")
                )
                terminal = replace(
                    prev,
                    exit_code=rc,
            ...
            except Exception:  # noqa: BLE001  # nosec - notification cannot break reaping
                import logging as _logging

                _logging.getLogger(__name__).debug("[bg] shell completion notification failed", exc_info=True)
                try:
...
def read_background_output(
    ...
    *,
    ...
    except Exception:  # pragma: no cover - decode("replace") shouldn't raise  # nosec
        output_text = ""
