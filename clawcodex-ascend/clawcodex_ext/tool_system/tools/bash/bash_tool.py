...
from clawcodex_ext.utils.shell_resolver import build_cwd_wrapper, resolve_shell
...
from .utils import strip_leading_blank_lines, truncate_output
...
def _bash_check_permissions(
    ...
    if _mode == "bypassPermissions" or (
        _mode == "plan" and getattr(perm_ctx, "is_bypass_permissions_mode_available", False)
    ):
        return PermissionPassthroughResult()
    ...
    if analysis.is_complex:
        ...
        if eval_like is not None:
            semantics_reason = (
                f"`{eval_like}` executes its arguments as shell code, which cannot be statically analyzed"
            )
            ...
            semantics_reason = "Accesses /proc/*/environ, which may expose environment secrets of another process"
    ...
    if parse_reason is not None or semantics_reason is not None:
        return PermissionAskDecision(
            behavior="ask",
            message=(f"This command requires confirmation: {parse_reason or semantics_reason}"),
    ...
    if perm_ctx is not None and getattr(perm_ctx, "mode", None) == "acceptEdits":
        accept_edits = check_accept_edits_bash(command, cwd=cwd_str, allowed_roots=allowed_roots)
    ...
    if check_read_only_constraints(command, cwd=cwd_str, allowed_roots=allowed_roots):
...
def _exact_allow_rule(perm_ctx: Any, command: str) -> Any | None:
    ...
    try:
        ...
        stripped = command.strip()
        for rule_content, rule in get_rule_by_contents_for_tool(perm_ctx, "Bash", "allow").items():
...
def bash_command_safety_guard(command: str) -> None:
    ...
    try:
        ...
        raise
    except Exception:  # noqa: BLE001  # nosec — the guard must never break the tool
        import logging as _logging

        _logging.getLogger(__name__).debug("[sandbox] guard check failed", exc_info=True)
...
def _bash_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    try:
        ...
        if run_result.timed_out:
            ...
            stderr_with_marker = f"{timeout_marker} {existing_stderr}" if existing_stderr else timeout_marker
            return ToolResult(
                name=BASH_TOOL_NAME,
                output={
                    "cwd": str(cwd),
...
def _eco_tee_dir(context: ToolContext) -> Path | None:
    ...
    except Exception:  # noqa: BLE001  # nosec — recovery dir is best-effort
        return None
...
def _maybe_apply_eco(
    ...
    *,
    ...
    try:
        ...
        baseline = _assemble_bash_body(output.get("stdout", ""), output.get("stderr", ""))
    ...
    except Exception:  # noqa: BLE001  # nosec — eco must never break the Bash tool
        import logging as _logging
    ...
    if isinstance(output, dict):
        ...
        if output.get("backgroundTaskId") and not output.get("stdout"):
            return {
                "type": "tool_result",
                "tool_use_id": tool_use_id,
                "content": output.get("message", "") or f"Background task started: {output['backgroundTaskId']}",
            }
...
BashTool: Tool = build_tool(
    ...
    is_destructive=lambda _input: (
        not is_command_read_only((_input or {}).get("command", ""), shell=(_input or {}).get("shell", "auto"))
    ),
