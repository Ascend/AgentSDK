...
def _enter_plan_mode_check_permissions(tool_input: dict[str, Any], _context: ToolContext) -> PermissionResult:
...
def _exit_plan_mode_validate(tool_input: dict[str, Any], context: ToolContext) -> ValidationResult:
...
def _exit_plan_mode_check_permissions(tool_input: dict[str, Any], _context: ToolContext) -> PermissionResult:
...
def _exit_plan_mode_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    has_task_tool = any(getattr(t, "name", None) == "Agent" for t in (context.options.tools or []))
