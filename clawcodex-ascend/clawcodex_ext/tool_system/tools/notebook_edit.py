...
def _generate_cell_id() -> str:
    return "".join(random.choices(string.ascii_lowercase + string.digits, k=8))  # nosec
...
def _notebook_edit_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    if path.suffix.lower() != ".ipynb":
        raise ToolInputError(f"Not a notebook file: {path}. Use the Edit tool for non-.ipynb files.")

    if edit_mode not in ("replace", "insert", "delete"):
        raise ToolInputError(f"Invalid edit_mode: {edit_mode}. Must be 'replace', 'insert', or 'delete'.")

    if edit_mode == "insert" and not cell_type:
        raise ToolInputError("cell_type is required when edit_mode is 'insert'. Specify 'code' or 'markdown'.")
    ...
    if cell_id is not None:
        ...
        if target_index is None and edit_mode != "insert":
            raise ToolInputError(
                f"Cell not found: {cell_id}. Available cells: {len(cells)} (use cell-0 through cell-{len(cells) - 1})"
            )
        if target_index is None and edit_mode == "insert":
...
NotebookEditTool: Tool = build_tool(
    ...
    user_facing_name=lambda input_data: (
        f"Edit Notebook: {(input_data or {}).get('notebook_path', '')}" if input_data else "Edit Notebook"
    ),
