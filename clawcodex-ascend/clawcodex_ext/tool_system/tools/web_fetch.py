...
def _domain_suggestions(rule_content: str) -> tuple:
    ...
    return (
        PermissionUpdateAddRules(
            ...
            behavior="allow",
            rules=(PermissionRuleValue(tool_name="WebFetch", rule_content=rule_content),),
        ),
    )
...
def _check_permissions(tool_input: dict[str, Any], context: ToolContext) -> PermissionResult:
    ...
    if perm_ctx is not None:
        try:
            ...
            deny = get_rule_by_contents_for_tool(perm_ctx, "WebFetch", "deny").get(rule_content)
            if deny is not None:
            ...
            ask = get_rule_by_contents_for_tool(perm_ctx, "WebFetch", "ask").get(rule_content)
            if ask is not None:
            ...
            allow = get_rule_by_contents_for_tool(perm_ctx, "WebFetch", "allow").get(rule_content)
