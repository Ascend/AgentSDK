...
def _apply_write_gate(payload: dict[str, Any], summary: str) -> dict[str, Any] | None:
    ...
    except Exception:  # noqa: BLE001  # nosec — fail open, donor behavior
        return None
...
def _memory_call(tool_input: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    if operations:
        if not isinstance(operations, list):
            raise ToolInputError("operations must be a list of {action, content?, old_text?} objects.")
        ...
        return ToolResult(name=MEMORY_TOOL_NAME, output=store.apply_batch(target, operations))
    ...
    if action not in _VALID_ACTIONS:
        raise ToolInputError(f"Unknown action '{action}'. Use add, replace, or remove — or pass an 'operations' batch.")
...
def _memory_enabled() -> bool:
    ...
    except Exception:  # noqa: BLE001  # nosec — default on, donor parity
        return True
