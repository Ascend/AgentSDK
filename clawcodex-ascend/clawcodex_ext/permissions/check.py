...
def _allowed_roots_for_check(context: ToolPermissionContext, tool_use_context: Any | None) -> list[str]:
    ...
    *,
    ...
    if context.mode == "bubble":
        ...
        return PermissionDenyDecision(
            behavior="deny",
            message=(
                f"Permission required for {tool.name} in bubble mode; request must be escalated to the parent agent."
            ),
            decision_reason=AsyncAgentDecisionReason(
...
def has_permissions_to_use_tool_inner(
    ...
    *,
    ...
    if (
        ...
        if isinstance(edit_path, str) and edit_path and _path_in_working_roots(edit_path, context, tool_use_context):
...
def _with_default_suggestions(
    ...
    * only an ask the matcher manufactured from a ``passthrough`` tool result
    ...
    suggestions = default_session_suggestions(tool_name, tool_input, context, allowed_roots=allowed_roots)
    if suggestions:
...
def prepare_permission_matcher(rule_content: str) -> Callable[[str], bool]:
    ...
    if "*" in rule_content or "?" in rule_content:
        return lambda command: (
            not contains_unquoted_chaining(command) and fnmatch.fnmatch(command.strip(), rule_content)
        )
