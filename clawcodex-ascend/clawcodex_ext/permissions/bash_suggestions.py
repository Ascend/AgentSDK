...
NEVER_PREFIX_COMMANDS = BARE_SHELL_PREFIXES | EVAL_LIKE_BUILTINS | ZSH_DANGEROUS_BUILTINS
...
def suggestion_for_prefix(prefix: str) -> list[PermissionUpdate]:
    ...
    return [
        PermissionUpdateAddRules(
            ...
            behavior="allow",
            rules=(PermissionRuleValue(tool_name=BASH_TOOL_NAME, rule_content=f"{prefix}:*"),),
        )
    ]
...
def suggestion_for_exact_command(command: str) -> list[PermissionUpdate]:
    ...
    return [
        PermissionUpdateAddRules(
            destination="localSettings",
            behavior="allow",
            rules=(PermissionRuleValue(tool_name=BASH_TOOL_NAME, rule_content=command),),
        )
    ]
...
def _rule_value_for_subcommand(sub: str) -> PermissionRuleValue | None:
    ...
    if prefix:
        return PermissionRuleValue(tool_name=BASH_TOOL_NAME, rule_content=f"{prefix}:*")
...
def suggestions_for_compound_command(command: str) -> list[PermissionUpdate] | None:
    ...
    return [PermissionUpdateAddRules(destination="localSettings", behavior="allow", rules=rules)]
