    ...
    *,
    ...
    if isinstance(update, PermissionUpdateAddRules):
        ...
        for rule_str in existing:
            normalized = permission_rule_value_to_string(permission_rule_value_from_string(rule_str))
...
_NO_SESSION_OPTION_TOOLS: frozenset[str] = frozenset({"AskUserQuestion", "EnterPlanMode", "ExitPlanMode"})
...
def session_option_label(
    ...
    has_accept_edits = any(isinstance(u, PermissionUpdateSetMode) and u.mode == "acceptEdits" for u in suggestions)
...
def serialize_permission_update(update: Any) -> dict:
    ...
    if rules:
        out["rules"] = [
            {"tool_name": getattr(r, "tool_name", ""), "rule_content": getattr(r, "rule_content", None)} for r in rules
        ]
