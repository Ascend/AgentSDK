...
def enforce_dangerous_skip_permissions_safety(
    ...
    * Skipped when ``IS_SANDBOX`` or ``CLAUDE_CODE_BUBBLEWRAP`` is truthy.
    ...
    out.write("--dangerously-skip-permissions cannot be used with root/sudo privileges for security reasons\n")
    try:
        out.flush()
    except Exception:  # pragma: no cover - flushing test buffers is best-effort  # nosec B110
        pass
    raise SystemExit(1)
