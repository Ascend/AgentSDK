...
def _on_main_loop_model_change(old: AppState, new: AppState) -> None:
    ...
    try:
        _persist_settings_keys(
            ...
            model_provider=(_active_provider_name() if new.main_loop_model else ""),
        )
    except Exception:
        logger.exception("Failed to persist model to settings; in-memory value still active")
...
def expanded_view_from_config_bools(show_expanded_todos: bool, show_spinner_tree: bool) -> str:
...
def _on_expanded_view_change(old: AppState, new: AppState) -> None:
    ...
    todos, spinner = _EXPANDED_VIEW_TO_BOOLS.get(new.expanded_view, (False, False))
    try:
        _persist_config_keys(showExpandedTodos=todos, showSpinnerTree=spinner)
    except Exception:
...
def _on_advisor_model_change(old: AppState, new: AppState) -> None:
    ...
    try:
        ...
        logger.exception("Failed to persist advisor_model to settings; in-memory value still active")
...
def _on_advisor_provider_change(old: AppState, new: AppState) -> None:
    ...
    try:
        ...
        logger.exception("Failed to persist advisor_provider to settings; in-memory value still active")
...
def _on_advisor_client_mode_change(old: AppState, new: AppState) -> None:
    ...
    try:
        ...
        logger.exception("Failed to persist advisor_client_mode to settings; in-memory value still active")
...
def _on_advisor_enabled_change(old: AppState, new: AppState) -> None:
    ...
    try:
        ...
        logger.exception("Failed to persist advisor_enabled to settings; in-memory value still active")
...
def seed_app_state_from_settings(active_provider: str | None) -> AppState:
    ...
    if model and (not settings.model_provider or settings.model_provider != (active_provider or "")):
        model = None
    if model:
