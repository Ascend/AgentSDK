...
def count_tokens(text: str) -> int:
    ...
    if encoder is not None:
        try:
            ...
            return result
        except Exception as exc:  # noqa: BLE001 - fall back to a rough estimate
            logger.debug("Tokenizer failed; using rough estimate", exc_info=exc)
...
def rough_token_count_estimation_for_message(message: Any) -> int:
    ...
    if msg_type == "attachment":
        attachment = message.get("attachment") if isinstance(message, dict) else getattr(message, "attachment", None)
