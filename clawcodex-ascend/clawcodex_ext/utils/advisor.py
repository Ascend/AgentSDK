...
def execute_client_advisor(
    ...
    *,
    ...
    if not advisor_provider:
        return (
            False,
            "Advisor unavailable: advisor_provider is not set. Run /advisor <provider>:<model> to configure.",
            _zero_usage,
        )
    try:
        ...
        try:
            ...
            return (
                False,
                f"Advisor unavailable: provider {advisor_provider!r} is not configured in ~/.clawcodex/config.json.",
                _zero_usage,
            )
    ...
    try:
        ...
        record_api_usage(
            call_kwargs.get("model") or getattr(response, "model", None) or getattr(provider, "model", "unknown"),
            raw_usage,
        )
        ...
        logging.getLogger(__name__).debug("advisor cost recording failed", exc_info=True)
