...
def force_close_response(stream: Any) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Could not shut down stream socket", exc_info=exc)
        close = getattr(response, "close", None)
        if callable(close):
            close()
    except Exception as exc:  # noqa: BLE001 - close is best-effort
        logger.debug("Could not close stream response", exc_info=exc)
...
class StreamWatchdog:
    ...
    def __init__(
        ...
        *,
        ...
        # Downstream integration: propagate a terminal idle timeout through
        # the shared abort channel so the legacy TUI/REPL becomes responsive
        # immediately. Ping progress only re-arms the watchdog and never
        # reaches this abort path.
    ...
    def _cancel_locked(self) -> None:
        if self._timer is not None:
            try:
                self._timer.cancel()
            except Exception as exc:  # noqa: BLE001 - timer may already be stopped
                logger.debug("Could not cancel stream deadline timer", exc_info=exc)
            self._timer = None
        if self._warn_timer is not None:
            try:
                self._warn_timer.cancel()
            except Exception as exc:  # noqa: BLE001 - timer may already be stopped
                logger.debug("Could not cancel stream warning timer", exc_info=exc)
            self._warn_timer = None
    ...
    def _on_timeout(self, expected_timer: threading.Timer | None) -> None:
        ...
        with self._lock:
            ...
            if current is not None and self._last_bytes is not None and current > self._last_bytes:
                self._arm_locked()
                return
            self._fired.set()
            self._timer = None
            stream = self._stream
            fired_timeout_s = self._effective_timeout_locked()
        ...
        if self._abort_signal is not None and not self._abort_signal.aborted:
            self._abort_signal._fire(f"timeout_after_{fired_timeout_s}s")
