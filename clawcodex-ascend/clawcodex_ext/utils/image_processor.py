...
def _pil():
    ...
    try:
        ...
        raise ImageProcessingError("Pillow is not installed. Add 'Pillow>=10.0' to dependencies.") from e
...
def _log_image_event(subtype: str, **fields: Any) -> None:
    ...
    try:
        ...
        logger.debug("Could not emit image-processing analytics", exc_info=exc)
...
def _encode_image(img: Any, media_type: str, quality: int | None = None) -> bytes:
    ...
    if media_type not in _SUPPORTED_ENCODE_TYPES:
        raise ImageProcessingError(
            f"Unsupported image encoding type: {media_type}. Supported: {sorted(_SUPPORTED_ENCODE_TYPES)}"
        )
    buf = io.BytesIO()
...
def maybe_resize_image(
    ...
    if original_size <= IMAGE_TARGET_RAW_SIZE and orig_w <= IMAGE_MAX_WIDTH and orig_h <= IMAGE_MAX_HEIGHT:
        return ResizeResult(
            data=buf,
...
def compress_image_to_byte_budget(
    ...
    last_attempt: bytes | None = None  # noqa: F841 - retained for the pending best-effort JPEG fallback
    last_attempt_dims: tuple[int, int] | None = None  # noqa: F841 - paired with the pending fallback result
    for scale in (1.0, 0.75, 0.5, 0.25):
        ...
        for quality in (80, 50):
            try:
                ...
                continue
            last_attempt = attempt  # noqa: F841 - retained for the pending best-effort JPEG fallback
            last_attempt_dims = (target_w, target_h)  # noqa: F841 - paired with the pending fallback result
    ...
    if sniffed_media == "image/png":
        ...
        except Exception as exc:  # noqa: BLE001 - optional palette compression
            logger.debug("Palette compression fallback failed", exc_info=exc)
