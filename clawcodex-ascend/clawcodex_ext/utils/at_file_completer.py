...
_AT_TOKEN_CHAR = r"[\w\-./\\()\[\]~:]"  # nosec B105 - regex character class, not a credential
...
class AtFileCompleter(Completer):
    ...
    def _candidates_snapshot(self) -> tuple[list[str], list[int]]:
        ...
        if self._cache and self._index_done_event.is_set() and (now - self._cache_built_at) < _CACHE_TTL_SECONDS:
