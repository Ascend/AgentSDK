...
extension layer per Phase 2-B (see ``DECOUPLING_PLAN.md`` §3.6). The
...
def _get_agent_type(agent: Any) -> str | None:
    ...
    agent_type = getattr(agent, "agent_type", None) or (agent.get("agent_type") if isinstance(agent, dict) else None)
