...
class ImageSizeError(Exception):
    ...
    def __init__(self, oversized: list[tuple[int, int]]):
        ...
        super().__init__(f"{len(oversized)} image(s) exceed the Anthropic API size limit: " + "; ".join(msg_parts))
...
def validate_images_for_api(messages: list[Any]) -> None:
    ...
    if oversized:
        ...
        try:
            ...
            logger.debug("Could not emit image-validation analytics", exc_info=exc)
