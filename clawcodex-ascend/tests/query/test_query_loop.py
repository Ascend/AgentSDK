...
from src.types.content_blocks import TextBlock, ToolResultBlock
...
class TestQueryLoopSingleTurn(unittest.TestCase):
    ...
    def test_max_turns_limit(self):
        ...
        max_turns_msgs = [
            m for m in collected if isinstance(m, SystemMessage) and getattr(m, "subtype", None) == "max_turns_reached"
        ]
...
class TestQueryLoopImageSizeError(unittest.TestCase):
    ...
    def test_image_size_error_from_provider_yields_media_size_assistant_message(self):
        ...
        content = err.content
        text = content if isinstance(content, str) else "".join(b.text for b in content if isinstance(b, TextBlock))
