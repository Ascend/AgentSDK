...
class TestPhaseBPromptTooLongRecovery(unittest.TestCase):
    ...
    def test_ptl_message_withheld_from_stream(self):
        ...
        ptl_messages = [
            m
            for m in collected
            if isinstance(m, AssistantMessage) and getattr(m, "_api_error", None) == "prompt_too_long"
        ]
        self.assertEqual(
            ptl_messages,
            [],
    ...
    def test_ptl_compact_failure_surfaces_terminal(self):
        ...
        provider = MagicMock()
        provider.chat_stream_response.side_effect = NotImplementedError()
        provider.chat.side_effect = lambda *a, **kw: (_ for _ in ()).throw(Exception("Prompt is too long"))
    ...
    def test_media_size_message_withheld_and_recovers(self):
        ...
        media_msgs = [
            m for m in collected if isinstance(m, AssistantMessage) and getattr(m, "_api_error", None) == "media_size"
        ]
...
class TestImageUnsupportedClassification(unittest.TestCase):
    ...
    def test_openrouter_404_yields_tagged_api_error(self):
        ...
        tagged = [
            m
            for m in collected
            if isinstance(m, AssistantMessage) and getattr(m, "_api_error", None) == "image_unsupported"
        ]
        self.assertEqual(
            len(tagged),
            1,
