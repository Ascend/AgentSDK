...
def test_flush_returns_false_when_no_pending(cursor: DeclaredCursor, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CLAWCODEX_DISABLE_DECLARED_CURSOR", raising=False)
    buf: list[str] = []
    cursor.set_writer(_capture_writer(buf))
...
def test_flush_idempotent_until_next_declaration(cursor: DeclaredCursor, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CLAWCODEX_DISABLE_DECLARED_CURSOR", raising=False)
    buf: list[str] = []
    cursor.set_writer(_capture_writer(buf))
...
def test_unregister_re_arms_pending_emit(cursor: DeclaredCursor, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_disable_env_kills_emission(cursor: DeclaredCursor, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_writer_exception_swallowed(cursor: DeclaredCursor, monkeypatch: pytest.MonkeyPatch) -> None:
