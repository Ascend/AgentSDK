...
async def test_message_selector_escape_resolves_cancel():
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        screen = MessageSelectorScreen(messages=[TranscriptMessage(index=0, kind="user", text="hi")])
        fut = _push(app, screen)
        await pilot.pause()
...
def test_parse_unified_diff_counts():
    ...
    add_lines = [line for line in lines if line.kind == "add"]
...
def test_parse_structured_patch_threads_line_numbers():
    ...
    remove_lines = [line for line in lines if line.kind == "remove"]
