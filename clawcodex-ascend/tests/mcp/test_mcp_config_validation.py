...
class TestValidateServerConfig:
    ...
    def test_stdio_non_list_args_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "py", "args": "not-a-list"})
        assert config is None
    ...
    def test_stdio_non_string_arg_element_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "py", "args": ["ok", 123]})
        assert config is None
        assert any("args[1]" in e for e in errors), errors

    def test_stdio_non_string_env_value_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "py", "env": {"X": 123}})
        assert config is None
    ...
    def test_stdio_non_dict_env_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "py", "env": "not-a-dict"})
        assert config is None
    ...
    def test_http_non_string_auth_server_metadata_url_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "http", "url": "https://x", "authServerMetadataUrl": 123})
        assert config is None
    ...
    def test_unknown_transport_type_returns_friendly_error(self) -> None:
        ...
        assert config is None
        assert any("unknown transport type" in e and "stdio" in e and "sse" in e and "http" in e for e in errors), (
            errors
        )
    ...
    def test_sse_non_string_headers_value_returns_error(self) -> None:
        config, errors = validate_server_config({"type": "sse", "url": "https://x", "headers": {"X": 1}})
        assert config is None
    ...
    def test_valid_stdio_with_type(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "python", "args": ["-m", "server"]})
        assert errors == []
    ...
    def test_valid_stdio_with_env(self) -> None:
        config, errors = validate_server_config({"type": "stdio", "command": "py", "env": {"FOO": "bar"}})
        assert errors == []
        assert isinstance(config, McpStdioServerConfig)
