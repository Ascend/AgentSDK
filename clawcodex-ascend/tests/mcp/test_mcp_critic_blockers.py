...
class TestAuthProviderWiring:
    ...
    async def test_connect_to_server_passes_auth_provider_to_client(self):
        ...
        with patch(
            ...
            client, conn = await connect_to_server("test-server", config, auth_provider=auth_provider)
...
class TestRedirectUriLocalhost:
    ...
    def test_xaa_idp_login_builds_localhost_redirect_uri(self):
        ...
        assert "http://localhost:" in source, "xaa_idp_login.acquire_idp_id_token must build a localhost redirect_uri"
        assert "http://127.0.0.1:" not in source, "xaa_idp_login.acquire_idp_id_token must not use 127.0.0.1 literal"
...
class TestAsyncTokenEndpoint:
    ...
    async def test_exchange_code_does_not_block_event_loop(self, tmp_path):
        ...
        assert observed_during_post, (
            "concurrent task did NOT progress while exchange_code's POST was awaiting — event loop was blocked"
        )
