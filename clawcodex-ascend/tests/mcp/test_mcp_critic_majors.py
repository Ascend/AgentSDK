...
def _build_jwt_with_exp(exp: int) -> str:
    ...
    payload = base64.urlsafe_b64encode(json.dumps({"exp": exp, "sub": "user"}).encode()).rstrip(b"=").decode()
...
class TestToolWrapperInputValidation:
    ...
    def test_missing_required_field_returns_structured_error(self):
        ...
        client = MagicMock()
        client.call_tool = AsyncMock(side_effect=AssertionError("server should not be called when validation fails"))
        tool = self._build_tool(
            {
                "type": "object",
                "properties": {"name": {"type": "string"}},
