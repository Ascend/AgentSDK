...
def test_empty_block_list_gets_wrapper_marker_block() -> None:
    ...
    assert out[0]["content"] == [{"type": "text", "text": "", "cache_control": {"type": "ephemeral"}}]
...
def test_non_dict_last_block_is_wrapped_into_text_block() -> None:
    ...
    out = add_cache_breakpoints(messages)
    assert out[0]["content"] == [{"type": "text", "text": "bare-string", "cache_control": {"type": "ephemeral"}}]
...
def test_unknown_scalar_content_coerced_via_str() -> None:
    ...
    out = add_cache_breakpoints(messages)
    assert out[0]["content"] == [{"type": "text", "text": "42", "cache_control": {"type": "ephemeral"}}]
