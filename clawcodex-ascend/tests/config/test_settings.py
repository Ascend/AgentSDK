...
from src.settings.change_detector import SettingsChangeDetector
...
class TestValidation:
    ...
    def test_permission_rule_empty_string_in_allow(self):
        ...
        s = SettingsSchema(permissions=PermissionsConfig(rules={"allow": ["Bash", ""], "deny": [], "ask": []}))
        errors = validate_settings(s)
...
class TestManagedPath:
    def test_returns_none_when_no_managed_file(self):
        with patch.dict(os.environ, {}, clear=False):
            with patch(
                "os.environ.get",
                side_effect=lambda k, *a: None if k == "CLAUDE_MANAGED_SETTINGS_PATH" else os.environ.get(k, *a),
            ):
