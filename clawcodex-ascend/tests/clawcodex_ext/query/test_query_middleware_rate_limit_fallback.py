...
"""Contract test for ``clawcodex_ext/query/query_middleware.py``.

The fork extracted the rate-limit / throttling logic out of
``src/query/query.py`` into ``clawcodex_ext/query/query_middleware.py`` so the
upstream query loop stays free of orchestrator-specific concerns. This
test pins the contract between the call sites in ``clawcodex_ext/query/query.py``
...
from clawcodex_ext.query.query_middleware import (
