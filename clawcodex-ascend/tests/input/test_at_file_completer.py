...
def _completions(completer: AtFileCompleter, text: str) -> list[tuple[str, int, str]]:
    ...
    return [(c.text, c.start_position, c.display_text) for c in completer.get_completions(doc, None)]
...
class TestScoreBoundRejection:
    ...
    def test_under_filled_top_k_invokes_subsequence_score(self, monkeypatch):
        ...
        assert call_count[0] == 3, (
            f"WI-3.3 must NOT skip when top-K is under-filled; expected 3 _subsequence_score calls, got {call_count[0]}"
        )
