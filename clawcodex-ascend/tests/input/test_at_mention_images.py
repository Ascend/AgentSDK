...
class TestImageAttachmentCompressionFallback(unittest.TestCase):
    ...
    def test_oversize_image_compressed_under_api_limit(self) -> None:
        ...
        self.assertGreater(
            ...
            "test setup: random-noise PNG must exceed the API cap for the compression branch to engage",
        )
        ...
        self.assertLessEqual(
            b64_len,
            API_IMAGE_MAX_BASE64_SIZE,
            f"Compression fallback failed to bring base64 under the API limit: {b64_len} > {API_IMAGE_MAX_BASE64_SIZE}",
        )
