...
class TestG2JitterConfig:
    ...
    # ---- Regression: issue #3 — floor > max must not breach max ----

    def test_validate_clamps_floor_to_max(self) -> None:
        """When floor > max, validate must clamp floor down to max."""
        cfg = CronJitterConfig(one_shot_floor_ms=600_000, one_shot_max_ms=60_000)
        clamped = validate_jitter_config(cfg)
        assert clamped.one_shot_floor_ms <= clamped.one_shot_max_ms

    def test_lead_respects_max_when_floor_exceeds_max(self) -> None:
        """floor=600000, max=60000 → lead must be <= 60000 after validation."""
        cfg = validate_jitter_config(CronJitterConfig(one_shot_floor_ms=600_000, one_shot_max_ms=60_000))
        for task_id in ["aaaa1111", "bbbb2222", "cccc3333"]:
            lead = one_shot_lead_ms(task_id, cfg)
            assert lead <= cfg.one_shot_max_ms

    # ---- Regression: issue #4 — int() on non-numeric strings ----

    def test_jitter_config_from_dict_non_numeric_int_fields(self) -> None:
        """JSON with string values in int fields must not crash; falls back to defaults."""
        cfg = jitter_config_from_dict({"recurring_cap_ms": "abc", "one_shot_max_ms": "xyz"})
        # Should fall back to defaults, not raise
        assert cfg.recurring_cap_ms == validate_jitter_config(CronJitterConfig()).recurring_cap_ms
        assert cfg.one_shot_max_ms == validate_jitter_config(CronJitterConfig()).one_shot_max_ms

    def test_load_jitter_config_survives_bad_int_field(self, tmp_path: Path) -> None:
        """load_jitter_config must not crash when JSON int fields are non-numeric."""
        config_path = tmp_path / ".clawcodex" / "cron" / "jitter_config.json"
        config_path.parent.mkdir(parents=True)
        config_path.write_text(json.dumps({"recurringCapMs": "not-a-number"}), encoding="utf-8")
        cfg = load_jitter_config(tmp_path)
        assert cfg.recurring_cap_ms == validate_jitter_config(CronJitterConfig()).recurring_cap_ms

    # ---- Regression: issue #5 — JSON enabled="false" must be falsy ----

    def test_jitter_config_from_dict_enabled_string_false(self) -> None:
        """JSON ``"enabled": "false"`` must parse as falsy (not True)."""
        cfg = jitter_config_from_dict({"enabled": "false"})
        assert cfg.enabled is False

    def test_jitter_config_from_dict_enabled_string_true(self) -> None:
        cfg = jitter_config_from_dict({"enabled": "true"})
        assert cfg.enabled is True

    def test_jitter_config_from_dict_enabled_bool(self) -> None:
        cfg = jitter_config_from_dict({"enabled": False})
        assert cfg.enabled is False

    # ---- Regression: issue #6 — jitter_frac must be in [0, 1) ----

    def test_jitter_frac_never_returns_one(self) -> None:
        from clawcodex_ext.cron_system.jitter import jitter_frac

        ids = ["a", "b", "c", "x" * 64] + [str(i) for i in range(100)]
        for task_id in ids:
            frac = jitter_frac(task_id)
            assert 0.0 <= frac < 1.0

    # ---- Regression: issue #29 — CronTask jitter default via field(default_factory) ----

    def test_cron_task_jitter_defaults_to_config(self) -> None:
        """CronTask created without explicit jitter must get a valid default config."""
        from clawcodex_ext.cron_system.models import CronTask, _default_jitter_config

        task = CronTask(id="t1", cron="*/5 * * * *", prompt="p")
        assert task.jitter is not None
        assert task.jitter.enabled == _default_jitter_config().enabled
...
class TestG4Permanent:
    ...
    # ---- Regression: issue #10 — multiple unrelated permanent tasks coexist ----

    def test_write_permanent_allows_unrelated_tasks(self, tmp_path: Path) -> None:
        """Two permanent tasks with different cron AND prompt must coexist."""
        task1, created1 = write_permanent_task_if_missing(tmp_path, cron="0 9 * * *", prompt="morning checkin")
        task2, created2 = write_permanent_task_if_missing(tmp_path, cron="0 22 * * *", prompt="evening backup")
        assert created1 is True
    ...
    # ---- Regression: issue #12 — case-insensitive matching ----

    def test_write_permanent_case_insensitive_match(self, tmp_path: Path) -> None:
        """Same cron/prompt with different case should be treated as duplicate."""
        task1, created1 = write_permanent_task_if_missing(tmp_path, cron="0 9 * * *", prompt="Morning Checkin")
        task2, created2 = write_permanent_task_if_missing(tmp_path, cron="0 9 * * *", prompt="morning checkin")
        assert created1 is True
        assert created2 is False
        assert task2.id == task1.id


# ---- Regression: issue #11 — one-shot tasks use one-shot jitter path ----


def test_add_cron_task_one_shot_uses_backward_lead(tmp_path: Path) -> None:
    """One-shot tasks (recurring=False) should use the backward-lead jitter path."""
    from clawcodex_ext.cron_system.jitter import one_shot_jittered_next_cron_run_ms
    from clawcodex_ext.cron_system.parser import parse_cron_expression

    cfg = CronJitterConfig(enabled=True, one_shot_max_ms=60_000, one_shot_minute_mod=30)
    ...
    expected = one_shot_jittered_next_cron_run_ms(task.id, fields, datetime.fromtimestamp(created_at / 1000), cfg)
    ...
    def test_prune_skips_permanent(self, tmp_path: Path) -> None:
        ...
        # Mark it as expired
        def write_cron_tasks_local(tasks):
            return _write_cron_tasks(tmp_path, tasks)
        ...
        prune_expired_recurring_tasks(tmp_path, at_ms=3_000)
...
class TestG5LockImprovements:
    ...
    def test_register_lock_cleanup_unregister(self) -> None:
        ...
        unregister = register_lock_cleanup(lambda: calls.append("a"))
        unregister()
        # release_all_locks snapshots the list at call time, so this asserts
        # that subsequent release_all_locks calls do not re-invoke the
    ...
    # ---- Regression: issue #9 — stale age must not override live PID ----

    def test_stale_age_does_not_recover_live_pid(self, tmp_path: Path) -> None:
        """A lock whose age exceeds stale_after_ms but whose PID is alive
        and passes identity check must NOT be recovered.
        """
        lock_path = tmp_path / ".clawcodex" / "cron" / "scheduled_tasks.lock"
        lock_path.parent.mkdir(parents=True)
        lock_path.write_text(
            json.dumps(
                {
                    "sessionId": "other",
                    "pid": os.getpid(),  # alive
                    "acquiredAt": 1,  # very old
                }
            ),
            encoding="utf-8",
        )
        lock = CronTaskLock(tmp_path, "fresh", stale_after_ms=1)
        # PID is alive and passes default validator → must NOT acquire
