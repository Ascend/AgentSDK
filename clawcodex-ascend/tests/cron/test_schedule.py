...
# ---- Regression: issue #17 — format_manual_fire_result distinguishes not-found ----


def test_format_manual_fire_result_not_found(tmp_path) -> None:
    output = format_manual_fire_result("missing", None, not_found=True)
    assert "does not exist" in output


def test_format_manual_fire_result_queued(tmp_path) -> None:
    output = format_manual_fire_result("abc123", None, not_found=False)
    assert "queued or running" in output
