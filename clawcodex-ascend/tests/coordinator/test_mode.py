...
def test_is_coordinator_mode_true_for_truthy_env(truthy: str, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CLAUDE_CODE_COORDINATOR_MODE", truthy)
    assert is_coordinator_mode() is True


@pytest.mark.parametrize("falsy", ["0", "false", "no", "off", "", "  "])
def test_is_coordinator_mode_false_for_falsy_env(falsy: str, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_user_context_default_branch_sorted(
    ...
    ctx = get_coordinator_user_context()
    body = ctx["workerToolsContext"]
    line = next(candidate for candidate in body.splitlines() if "Workers spawned" in candidate)
