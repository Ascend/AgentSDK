...
def test_permission_cascade_matrix(
    ...
    assert perm.should_avoid_permission_prompts is exp_avoid, (
        f"mode={effective_mode} is_async={is_async} parent_avoids={parent_avoids} -> expected should_avoid={exp_avoid}"
    )
    assert perm.await_automated_checks_before_dialog is exp_await, (
        f"mode={effective_mode} is_async={is_async} parent_avoids={parent_avoids} -> expected await={exp_await}"
    )
