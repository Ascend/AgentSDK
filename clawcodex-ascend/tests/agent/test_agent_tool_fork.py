...
def test_fork_recursive_guard_via_query_source(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _set_fork_env(monkeypatch, enabled=True)
    registry = build_default_registry(provider=object())
    context = _make_interactive_context(tmp_path)
...
def test_fork_recursive_guard_via_message_scan(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _set_fork_env(monkeypatch, enabled=True)
    registry = build_default_registry(provider=object())
    context = _make_interactive_context(tmp_path)
...
def test_fork_routing_includes_parent_assistant_clone(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _set_fork_env(monkeypatch, enabled=True)
    registry = build_default_registry(provider=object())
    context = _make_interactive_context(tmp_path)
...
def test_fork_disabled_in_non_interactive_session(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _set_fork_env(monkeypatch, enabled=True)
    registry = build_default_registry(provider=object())
    context = _make_interactive_context(tmp_path)
...
def test_fork_uses_rendered_system_prompt_when_present(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_falls_back_to_custom_system_prompt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_falls_back_to_active_agent_def_prompt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_parent_system_prompt_none_when_no_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_appends_worktree_notice_when_worktree_root_set(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_skips_worktree_notice_when_worktree_root_unset(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fork_skips_worktree_notice_when_root_matches_cwd(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    if isinstance(last.content, list):
        assert any(isinstance(b, TextBlock) and "<fork-boilerplate>" in b.text for b in last.content)
    else:
        assert "<fork-boilerplate>" in last.content


def test_fork_worktree_notice_does_not_trip_recursion_guard(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
