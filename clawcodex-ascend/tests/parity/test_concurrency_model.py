...
class TestSyntheticErrorMessages(unittest.TestCase):
    ...
    def test_sibling_error_message(self) -> None:
        ...
        executor._errored_tool_description = "Bash(ls)"
        msg = executor._create_synthetic_error_message("tool_1", "sibling_error", AssistantMessage())
        content = msg.content
        self.assertIsInstance(content, list)
        self.assertTrue(
            any("Cancelled" in str(block.get("content", "")) for block in content if isinstance(block, dict))
        )
    ...
    def test_user_interrupted_message(self) -> None:
        ...
        msg = executor._create_synthetic_error_message("tool_1", "user_interrupted", AssistantMessage())
        content = msg.content
        self.assertIsInstance(content, list)
    ...
    def test_streaming_fallback_message(self) -> None:
        ...
        msg = executor._create_synthetic_error_message("tool_1", "streaming_fallback", AssistantMessage())
        content = msg.content
        self.assertIsInstance(content, list)
        self.assertTrue(
            any("Streaming fallback" in str(block.get("content", "")) for block in content if isinstance(block, dict))
        )
...
class TestUnknownToolHandling(unittest.TestCase):
    ...
    def test_unknown_tool_immediate_error(self) -> None:
        ...
        msg = results[0]
        self.assertTrue(
            any("No such tool" in str(block.get("content", "")) for block in msg.content if isinstance(block, dict))
        )
