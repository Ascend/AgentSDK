...
class TestE2EFileRead(unittest.TestCase):
    ...
    def test_multi_tool_batch_preserves_tool_result_pairing(self) -> None:
        ...
        tool_result_msgs = [m for m in results if m.content and hasattr(m.content[0], "tool_use_id")]
        ...
        for m in tool_result_msgs:
            self.assertFalse(
                ...
                f"tu_{m.content[0].tool_use_id} regression: got error placeholder instead of real result",
            )
            ...
            for block in content:
                ...
                if block.get("type") == "tool_result":
                    ...
                    self.assertNotIn(
                        "Tool result missing",
                        str(body),
                        f"{tu_id}: API would receive synthetic placeholder instead of the real tool_result",
                    )
                    found_ids.add(tu_id)
        self.assertEqual(found_ids, {"tu_a", "tu_b"}, "API payload missing one or both tool_results")
    ...
    def test_dispatch_preserves_image_under_aggregate_threshold_pressure(self) -> None:
        ...
        self.assertIsInstance(
            body,
            list,
            "Aggregate-pressure path collapsed image to text instead of short-circuiting via _has_image_block",
        )
...
class TestE2EGlobGrep(unittest.TestCase):
    ...
    def test_grep_no_match_returns_empty(self) -> None:
        ...
        result = self.registry.dispatch(
            ToolCall(
                name="Grep",
                input={"pattern": "nonexistent_string_xyz", "path": str(self.root)},
            ),
            self.ctx,
        )
        self.assertFalse(result.is_error)
