...
class TestTwoPhaseWatchdog(unittest.TestCase):
    ...
    def test_survives_first_event_wait_then_fires_on_inter_event_idle(self):
        ...
        watchdog = StreamWatchdog(stream, timeout_s=0.1, first_event_timeout_s=0.8)
        watchdog.arm()
    ...
    def test_first_event_timeout_still_fires_a_truly_dead_stream(self):
        ...
        stream = MagicMock()
        stream.response = resp
        watchdog = StreamWatchdog(stream, timeout_s=0.1, first_event_timeout_s=0.2)
        watchdog.arm()
...
class TestPingAwareLiveness(unittest.TestCase):
    ...
    def test_byte_progress_prevents_fire(self):
        ...
        watchdog = StreamWatchdog(stream, timeout_s=0.08, first_event_timeout_s=0.08)
        watchdog.arm()
...
class TestWatchdogIntegrationWithProvider(unittest.TestCase):
    ...
    def test_watchdog_fire_retries_stream_then_succeeds(self):
        ...
        with (
            _patch.dict(
                os.environ, {"CLAUDE_STREAM_IDLE_TIMEOUT_MS": "50", "CLAUDE_STREAM_FIRST_EVENT_TIMEOUT_MS": "50"}
            ),
...
class TestChatExplicitTimeout(unittest.TestCase):
    ...
    def _chat_create_kwargs(self, env=None):
        ...
        fake_client = MagicMock()
        fake_client.messages.create.return_value = MagicMock(content=[], usage=None, stop_reason="end_turn", model="m")
        provider = AnthropicProvider(api_key="test")
        provider.client = fake_client
    ...
    def test_caller_supplied_timeout_wins(self):
        ...
        self.assertEqual(fake_client.messages.create.call_args.kwargs.get("timeout"), 42.0)
    ...
    def test_unblocks_a_reader_parked_on_a_real_socket(self):
        ...
        def blocked_reader():
            try:
                ...
                pass  # Socket closure is the expected way to unblock this reader.
            unwound.set()
