...
def test_dispatch_pre_tool_abort_returns_reject_message(tmp_path: Path) -> None:
    ...
    registry = MagicMock()
    registry.dispatch = MagicMock(side_effect=AssertionError("registry must not be hit when abort is already tripped"))
...
def test_sibling_error_does_not_override_with_reject_message(tmp_path: Path) -> None:
    ...
    assert sentinel in str(tool_result.content), "the real tool failure payload must reach the model unchanged"
    assert tool_result.is_error is True
