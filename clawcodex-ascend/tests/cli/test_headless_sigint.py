...
def _patch_provider_only(monkeypatch, *, on_chat=None):
    ...
    def _fake_provider_class(provider_name):
        return lambda api_key, base_url=None, model=None: _CancellingProvider(api_key, base_url, model, on_chat=on_chat)
...
def test_headless_mid_tool_cancel_emits_cancelled_result(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_headless_json_mode_reports_cancelled_subtype(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_install_sigint_handler_off_main_thread_is_noop() -> None:
    ...
    def _thread_body() -> None:
        ...
        except Exception as exc:  # pragma: no cover — failure path  # noqa: BLE001
            result["error"] = exc
