...
def test_cli_provider_list_exits_0():
    ...
    for name in ("anthropic", "openai"):
        assert name.lower().replace("-", "") in proc.stdout.lower().replace("-", "") or name.lower().replace(
            "-", ""
        ) in proc.stderr.lower().replace("-", ""), f"Expected {name!r} in provider list output"
...
def test_tui_entrypoint_imports():
    ...
    from clawcodex_ext.tui.entrypoint import _run_tui_with_app, run_tui
