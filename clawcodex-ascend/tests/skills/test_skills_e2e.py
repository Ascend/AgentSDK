...
FIXTURES_ROOT = Path(__file__).parents[1] / "fixtures" / "skills"
...
def test_commit_helper_renders_all_substitutions(
    ...
    assert prompt.startswith(expected_header), (
        f"missing/mangled base-dir header.\n  expected start: {expected_header!r}\n  actual start:   {prompt[:200]!r}"
    )
    ...
    skill = {item.name: item for item in get_all_skills(project_root=project_with_fixtures)}["commit-helper"]
    assert skill.allowed_tools == [
        "Bash",
        "Read",
    ], f"allowed-tools metadata was not loaded: {skill!r}"
...
def test_frontend_add_component_namespaced_invocation(
    ...
    assert by_name["frontend:add-component"].when_to_use == ("When the user asks to add a new React component.")
    ...
    assert "src/components/Button.tsx" in rendered_prompt(result)
...
def test_lint_py_not_invokable_until_activated(
    ...
    assert "lint-py" not in names, f"conditional skill must NOT appear before path activation; got: {sorted(names)}"
    ...
    assert "lint-py" in activated, f"conditional skill should activate on matching path; activated={activated}"
    ...
    assert "lint-py" in names_after, (
        f"after activation, lint-py should be reachable via get_all_skills; got: {sorted(names_after)}"
    )
...
def test_lint_py_after_activation_runs_shell_block_through_skilltool(
    ...
    assert after_marker.startswith("/"), f"shell exec should have produced a path; got: {after_marker!r}"
