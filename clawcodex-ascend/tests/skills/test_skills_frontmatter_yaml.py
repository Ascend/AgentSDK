...
class TestFieldValidators:
    def test_model_inherit_returns_none(self) -> None:
        ...
        assert result["model"] is None, (
            "`model: inherit` must yield None so callers fall back to the active session model (TS parity)."
        )
    ...
    def test_effort_invalid_drops_with_warning(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        assert result["effort"] is None, "invalid effort levels must be dropped (None), not preserved"
    ...
    def test_allowed_tools_preserves_paren_arg_pattern(self) -> None:
        ...
        assert result["allowed_tools"] == ["Bash(git diff:*)"], (
            "paren-arg pattern must be preserved as one entry; splitting on `:` would break Bash command rules"
        )
    ...
    def test_hooks_valid_event_round_trips_to_dict(self) -> None:
        hooks_in = {"PostToolUse": [{"matcher": "Write", "hooks": [{"type": "command", "command": "x"}]}]}
