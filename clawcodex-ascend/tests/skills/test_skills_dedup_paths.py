...
def test_symlinked_skill_collapses_to_single_entry(tmp_path: Path, isolated_home: Path) -> None:
    ...
    seen_files = {os.path.realpath(str(Path(s.base_dir) / "SKILL.md")) for s in skills if s.base_dir}
    assert len(seen_files) == 1, f"realpath dedup should collapse symlinked skills, got: {[s.name for s in skills]}"
...
def test_disable_policy_skills_excludes_managed_dir(
    ...
    assert all(s.name != "policyskill" for s in skills2), (
        f"CLAUDE_CODE_DISABLE_POLICY_SKILLS=1 must skip the managed dir; got: {[s.name for s in skills2]}"
    )


def test_bare_mode_skips_autodiscovery(tmp_path: Path, isolated_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project = tmp_path / "proj"
    _write_skill(
...
def test_conditional_skill_held_until_path_matches(tmp_path: Path, isolated_home: Path) -> None:
    ...
    skills = get_skill_dir_commands(str(project))
    assert all(s.name != "lintpy" for s in skills), "conditional skill must NOT appear in unconditional results"
    assert get_conditional_skill_count() >= 1

    # A non-matching activation must not flip it on.
    activated = activate_conditional_skills_for_paths([str(project / "docs" / "x.md")], str(project))
    assert activated == []
    assert "lintpy" not in {s.name for s in get_dynamic_skills()}

    # A matching activation flips it on.
    activated = activate_conditional_skills_for_paths([str(project / "src" / "foo" / "bar.py")], str(project))
...
def test_path_validity_guards_reject_dotdot_and_absolute(tmp_path: Path, isolated_home: Path) -> None:
    project = tmp_path / "proj"
    _write_skill(
    ...
    assert activated == [], "files outside the cwd (relpath starts with '..') must be ignored by the activation guard"
...
def test_dynamic_discovery_skips_gitignored_dirs(
    ...
    assert all("node_modules" not in d for d in new_dirs), f"gitignored skills dir leaked into discovery: {new_dirs}"
...
def test_dynamic_discovery_includes_non_ignored_dirs(tmp_path: Path, isolated_home: Path) -> None:
    ...
    assert any(str(nested_skills_dir) == d for d in new_dirs), (
        f"non-ignored nested .clawcodex/skills dir should appear in discovery; got: {new_dirs}"
    )
