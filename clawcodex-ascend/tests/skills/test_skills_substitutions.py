...
class TestSubstituteSkillDir:
    ...
    def test_no_base_dir_leaves_placeholder(self) -> None:
        ...
        assert substitute_skill_dir("Path: ${CLAUDE_SKILL_DIR}", None) == "Path: ${CLAUDE_SKILL_DIR}"
...
def test_skilltool_invocation_substitutes_all_placeholders(tmp_path: Path, isolated_home: Path) -> None:
...
def test_bundled_skill_invocation_does_not_get_base_dir_header(tmp_path: Path, isolated_home: Path) -> None:
    register_bundled_skill(
        BundledSkillDefinition(
            name="bcheck",
