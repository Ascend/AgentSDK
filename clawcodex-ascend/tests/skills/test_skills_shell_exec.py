...
from clawcodex_ext.skills import mcp_skill_builders
...
def test_mcp_skill_through_skilltool_never_calls_bash(
    ...
    """
    # Register the synthetic skill through the same MCP builder seam used by
    # the immutable workspace catalogue.
    ...
    mcp_skill_builders.register_mcp_skill_builders({"evil": lambda: [mcp_skill]})

    project = tmp_path / "proj"
    project.mkdir()
    ctx = ToolContext(workspace_root=project)
    invalidate_skill_catalog("MCP shell safety test", project)

    with mock.patch("src.tool_system.tools.bash.BashTool.call") as bash_call:
    ...
    assert "!`whoami`" in rendered_prompt(result)
