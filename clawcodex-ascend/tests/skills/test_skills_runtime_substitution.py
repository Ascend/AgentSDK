...
class TestSkillToolRuntimeIntegration(unittest.TestCase):
    ...
    def test_disk_skill_gets_base_dir_header(self) -> None:
        ...
        result = SkillTool.call({"skill": "hello"}, ctx)
        out = result.output
        prompt = rendered_prompt(result)
        self.assertTrue(out["success"])
        self.assertTrue(
            prompt.startswith("Base directory for this skill:"),
            f"prompt does not start with header: {prompt!r}",
        )
        self.assertIn(str(skills_dir / "hello"), prompt)
    ...
    def test_skill_dir_and_session_substitution(self) -> None:
        ...
        result = SkillTool.call({"skill": "echo"}, ctx)
        out = result.output
        prompt = rendered_prompt(result)
        self.assertTrue(out["success"])
        self.assertIn(f"DIR={skills_dir / 'echo'}", prompt)
        self.assertIn("SID=my-session-42", prompt)
    ...
    def test_bundled_skill_no_header(self) -> None:
        ...
        ctx = ToolContext(workspace_root=self.root)
        result = SkillTool.call({"skill": "b1", "args": "x"}, ctx)
        out = result.output
        prompt = rendered_prompt(result)
        self.assertTrue(out["success"])
        self.assertNotIn("Base directory for this skill", prompt)
        self.assertIn("BUNDLED-PROMPT:x", prompt)
    ...
    def test_argument_substitution_with_header(self) -> None:
        ...
        ctx = ToolContext(workspace_root=self.root)
        result = SkillTool.call({"skill": "greet", "args": "alice town"}, ctx)
        out = result.output
        prompt = rendered_prompt(result)
        self.assertTrue(out["success"])
        self.assertTrue(prompt.startswith("Base directory for this skill"))
        self.assertIn("Hello alice from town", prompt)
