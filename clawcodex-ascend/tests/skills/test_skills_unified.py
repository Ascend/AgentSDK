...
# ----------------------------------------------------------------------
# Test isolation: every test gets a clean ``HOME`` so ``~/.clawcodex``
# lookups don't leak from the developer's machine, and
...
def test_flat_project_skill_invokable_via_skilltool(tmp_path: Path, isolated_home: Path) -> None:
    ...
    assert "foo body" in rendered_prompt(result)
...
def test_nested_namespace_skill_invokable_as_colon_form(tmp_path: Path, isolated_home: Path) -> None:
    project = tmp_path / "proj"
    _write_skill(
    ...
    assert "Write a commit message" in rendered_prompt(result)
...
def test_nested_namespace_appears_in_get_all_skills(tmp_path: Path, isolated_home: Path) -> None:
    ...
    assert "deep:nested:thing" in by_name, f"expected nested namespace 'deep:nested:thing' in {sorted(by_name)}"
...
def test_bundled_skill_discoverable_through_skilltool(tmp_path: Path, isolated_home: Path) -> None:
    ...
    assert "bundled prompt: hello" in rendered_prompt(result)
    assert get_registered_skill("bundled-thing").loaded_from == "bundled"
...
def test_get_all_skills_returns_union_of_bundled_and_disk(tmp_path: Path, isolated_home: Path) -> None:
    project = tmp_path / "proj"
    _write_skill(
...
def test_env_var_user_skill_dir_is_reachable_via_skilltool(
    ...
    out = result.output
    assert out["success"] is True
    assert "body-from-env" in rendered_prompt(result)
    by_name = {skill.name: skill for skill in get_all_skills(project_root=project)}
    assert by_name["envskill"].loaded_from == "user"
...
def test_get_registered_skill_populated_after_get_all_skills(tmp_path: Path, isolated_home: Path) -> None:
    project = tmp_path / "proj"
    _write_skill(
