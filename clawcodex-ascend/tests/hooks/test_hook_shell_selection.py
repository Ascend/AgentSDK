...
class TestValidatorShell:
    def test_bash_accepted(self) -> None:
        errors = validate_hook_configs({"PreToolUse": [{"type": "command", "command": "x", "shell": "bash"}]})
        assert errors == []

    def test_powershell_accepted(self) -> None:
        errors = validate_hook_configs({"PreToolUse": [{"type": "command", "command": "x", "shell": "powershell"}]})
        assert errors == []

    def test_unknown_shell_emits_error(self) -> None:
        errors = validate_hook_configs({"PreToolUse": [{"type": "command", "command": "x", "shell": "fish"}]})
        assert len(errors) == 1
...
class TestExecutorPowerShellPath:
    ...
    @pytest.mark.asyncio
    async def test_blocking_error_preserves_command_in_message(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("clawcodex_ext.utils.shell_resolver.shutil.which", lambda _name: None)
