...
# pylint: disable=C0412,E0611,R1732,W0404,W0621
import io
import json
import os
import socket
...
class TestWebSearchTool(ToolSystemTests):
    ...
    def test_web_search_domain_filtering_blocked(self) -> None:
        ...
        with self._mock_tavily(hits):
            out = WebSearchTool.call({"query": "test", "blocked_domains": ["example.com"]}, self.ctx).output
        links = out["results"][-1]
    ...
    def test_web_search_domain_filtering_allowed(self) -> None:
        ...
        with self._mock_tavily(hits):
            out = WebSearchTool.call({"query": "test", "allowed_domains": ["example.com"]}, self.ctx).output
        links = out["results"][-1]
        self.assertEqual(len(links["content"]), 1)
        self.assertEqual(links["content"][0]["url"], "https://example.com/")
