...
# pylint: disable=E0611,R1732,W0404,W0621
import tempfile
import unittest
...
class TestPerMessageAggregateBudget(unittest.TestCase):
    ...
    def test_query_loop_resets_aggregate_each_turn(self):
        ...
        self.assertGreater(
            ...
            0,
            "WI-5.1 per-turn reset missing from query.py — tool_use_context.tool_result_chars_so_far = 0 not found",
        )
    ...
    def test_dispatch_concurrent_does_not_bypass_cap(self):
        ...
        async def run_parallel():
            async def dispatch_one(b):
                return await asyncio.to_thread(_dispatch_single_tool, b, fake_registry, ctx, [fake_tool])
        ...
        persisted_count = sum(1 for pair in results if "<persisted-output>" in pair[0].content[0].content)
        self.assertGreater(
            persisted_count,
            0,
