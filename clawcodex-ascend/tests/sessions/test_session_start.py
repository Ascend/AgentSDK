...
class TestInitializePromptCacheEligibility(unittest.TestCase):
    def test_explicit_inputs_take_precedence_over_env(self) -> None:
        with mock.patch.dict(os.environ, {"CLAUDE_CODE_USER_TYPE": "ant"}, clear=False):
            result = initialize_prompt_cache_eligibility(is_ant_user=False, is_subscriber=False, is_using_overage=False)
    ...
    def test_subscriber_not_overage_yields_eligible(self) -> None:
        result = initialize_prompt_cache_eligibility(is_ant_user=False, is_subscriber=True, is_using_overage=False)
        self.assertTrue(result)

    def test_subscriber_with_overage_yields_not_eligible(self) -> None:
        result = initialize_prompt_cache_eligibility(is_ant_user=False, is_subscriber=True, is_using_overage=True)
        self.assertFalse(result)
    ...
    def test_latch_is_one_shot(self) -> None:
        ...
        """
        first = initialize_prompt_cache_eligibility(is_ant_user=True, is_subscriber=False, is_using_overage=False)
        self.assertTrue(first)

        # Now call again with inputs that *would* yield False
        second = initialize_prompt_cache_eligibility(is_ant_user=False, is_subscriber=False, is_using_overage=True)
    ...
    def test_initialize_settles_state_for_should_1h_cache_ttl(self) -> None:
        ...
        """
        initialize_prompt_cache_eligibility(is_ant_user=False, is_subscriber=True, is_using_overage=False)
