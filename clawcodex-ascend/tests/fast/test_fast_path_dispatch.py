...
def _run_clawcodex_subcommand(*argv: str) -> tuple[int, dict]:
    ...
    if report_line is None:
        pytest.fail(f"subcommand {argv!r} produced no module report; stdout={proc.stdout!r} stderr={proc.stderr!r}")
...
def test_mcp_list_does_not_load_anthropic_sdk():
    ...
    rc, report = _run_clawcodex_subcommand("mcp", "list")
    assert not report["anthropic_sdk"], "WI-4.3 + WI-4.4: fast-path mcp must not load the anthropic SDK"
...
def test_flag_value_mcp_does_not_trigger_fast_path():
    ...
    assert not report["entrypoint_target"], "flag value 'mcp' must not be interpreted as a fast-path subcommand"
...
def test_short_flag_value_doctor_does_not_trigger_fast_path():
    ...
    assert not report["entrypoint_target"], "print-mode prompt 'doctor' must not trigger the doctor fast-path"
...
def test_prompt_value_daemon_does_not_trigger_fast_path():
    ...
    assert not report["entrypoint_target"], "--allowed-tools 'daemon' must not trigger the daemon fast-path"
