...
class TestQueryRunnerDebug(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stream_drains_queued_events_after_headless_completion(self) -> None:
        ...
        tool_events = [event for event in events if isinstance(event, (ToolCallEvent, ToolResultEvent))]
        ...
        self.assertTrue(any(isinstance(event, SessionComplete) and event.reason == "exit_code=1" for event in events))
    ...
    async def test_stream_backfills_tool_name_for_result_events(self) -> None:
        ...
        headless_results = [row for row in rows if row["stage"] == "headless.event" and row["kind"] == "tool_result"]
