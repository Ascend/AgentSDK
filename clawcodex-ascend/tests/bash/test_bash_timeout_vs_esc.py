...
def test_run_bash_with_abort_sets_only_timed_out_on_timeout(tmp_path: Path) -> None:
    ...
    assert elapsed < 3.0, (
        f"timeout supervisor should have killed the process well under the 3s sleep, took {elapsed:.2f}s"
    )
...
def test_bash_call_timeout_payload_shape(tmp_path: Path) -> None:
    ...
    assert result.is_error is False, (
        "_bash_call must mirror TS by setting is_error=False on timeout; the duration marker in stderr is the signal."
    )
