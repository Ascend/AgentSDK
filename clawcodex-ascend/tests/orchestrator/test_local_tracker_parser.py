...
class SplitFrontmatterTests(unittest.TestCase):
    ...
    def test_malformed_yaml_is_tolerated(self) -> None:
        # A broken frontmatter block (unclosed flow sequence) must not
        # raise: it degrades to empty metadata like the no-frontmatter
        # path, so one bad file cannot abort a whole directory scan.
        text = "---\nid: F-broken\nstate: [open\n---\nbody\n"
        metadata, body = _split_frontmatter(text)
        self.assertEqual(metadata, {})
        self.assertEqual(body, "body\n")

    def test_malformed_yaml_logs_warning_with_position(self) -> None:
        text = "---\nid: F-broken\nstate: [open\n---\nbody\n"
        with self.assertLogs("extensions.orchestrator.local_tracker.parser", level="WARNING") as captured:
            _split_frontmatter(text, path=Path("/tmp/issue.md"))
        self.assertTrue(
            any("malformed YAML frontmatter" in line and "/tmp/issue.md" in line for line in captured.output),
            captured.output,
        )
