...
class TestQueryIntegrationToolCalls(unittest.TestCase):
    ...
    def test_full_write_tool_roundtrip(self):
        ...
        tool_results = [m for m in collected if isinstance(m, UserMessage) and isinstance(m.content, list)]
...
class TestQueryIntegrationErrors(unittest.TestCase):
    ...
    def test_api_error_handled_gracefully(self):
        ...
        error_msgs = [
            m
            for m in collected
            if isinstance(m, AssistantMessage) and isinstance(m.content, str) and "error" in m.content.lower()
        ]
