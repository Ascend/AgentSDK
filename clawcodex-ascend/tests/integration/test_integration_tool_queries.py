                ...
                tool_uses=[
                    {
                        ...
                        "name": "Write",
                        "input": {
                            "file_path": str(self._ws / "README.md"),
                            "content": "hello",
                        },
                    }
                ],
...
class TestIntegrationPermissions:
    def test_write_md_blocked_by_default(self, tmp_path):
        result = _run(
            _PermissionDeniedWriteProvider(tmp_path),
            tmp_path,
            "Write docs",
            mode="default",
        )
...
class TestIntegrationREPLChat:
    ...
    def test_repl_chat_tool_creates_file(self, tmp_path):
        ...
        """
        pytest.skip("REPL provider patching does not propagate through the query() pipeline; needs a dedicated fixture")
        home = tmp_path / "home"
        work = tmp_path / "work"
        work.mkdir()
        config_file = self._make_config(home)
