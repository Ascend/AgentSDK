    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if path == "/v1/code/sessions":
            if create_session_status >= 400:
                return httpx.Response(create_session_status, json={"error": {"type": "create_failed"}})
...
async def test_init_schedules_token_refresh() -> None:
    ...
    with patch("src.bridge.remote_bridge_core.TokenRefreshScheduler.schedule_from_expires_in") as mock_sched:
        async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
            handle = await init_env_less_bridge_core(
                params,
                http_client=client,
