...
class TestTypeGuards:
    ...
    def test_is_sdk_control_request_requires_all_three_fields(self) -> None:
        assert is_sdk_control_request({"type": "control_request", "request_id": "r1", "request": {}})
...
class TestNormalizeControlMessageKeys:
    def test_known_camel_case_keys_get_snake_cased(self) -> None:
        out = normalize_control_message_keys({"requestId": "r1", "toolUseId": "tu", "workerEpoch": 5})
    ...
    def test_unknown_camel_case_passes_through_with_debug_log(self, caplog: pytest.LogCaptureFixture) -> None:
...
class TestExtractTitleText:
    ...
    def test_tool_use_result_returns_none(self) -> None:
        assert extract_title_text({"type": "user", "toolUseResult": {}, "message": {"content": "x"}}) is None

    def test_compact_summary_returns_none(self) -> None:
        assert extract_title_text({"type": "user", "isCompactSummary": True, "message": {"content": "x"}}) is None
